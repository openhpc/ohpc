...
#define CYCLIN(x, y, dimX, dimY) \
	LIN((x + dimX) % (dimX), (y + dimY) % (dimY), (dimX))
...
class Grid {
    ...
    public:
	Grid(int dimX, int dimY, int numGridsX, int numGridsY,
	     int numInitialPerturbations)
		: dimX_(dimX)
		, dimY_(dimY)
		, numGridsX_(numGridsX)
		, numGridsY_(numGridsY)
	{
	...
	void initGrid(int numInitialPerturbations)
	{
		for (int s = 0; s < numInitialPerturbations; s++) {
			...
			int xcenter = radius + rand() % (dimX_ * numGridsX_ -
							 2 * radius);
			int ycenter = radius + rand() % (dimY_ * numGridsY_ -
							 2 * radius);
		...
		for (int x = 0; x < dimX_; ++x) {
			for (int y = 0; y < dimY_; ++y) {
				myGridNext_[LIN2(x, y, dimX_)] =
					ks * (myGridCurr_[LIN2(x + 1, y, dimX_)] +
					      myGridCurr_[LIN2(x - 1, y, dimX_)] +
					      myGridCurr_[LIN2(x, y + 1, dimX_)] +
					      myGridCurr_[LIN2(x, y - 1, dimX_)] -
					      myGridCurr_[LIN2(x, y, dimX_)] *
						      4) -
					myGridPrev_[LIN2(x, y, dimX_)] +
					2 * myGridCurr_[LIN2(x, y, dimX_)];
			}
		}
	}
	...
	void exchangeEdges()
	{
		...
		int topRank =
			CYCLIN(myGridX_, myGridY_ + 1, numGridsX_, numGridsY_);
		int leftRank =
			CYCLIN(myGridX_ - 1, myGridY_, numGridsX_, numGridsY_);
		int rightRank =
			CYCLIN(myGridX_ + 1, myGridY_, numGridsX_, numGridsY_);
		int bottomRank =
			CYCLIN(myGridX_, myGridY_ - 1, numGridsX_, numGridsY_);
		...
		/*************** Recv ***************/
		// Top.
		MPI_Irecv(&(myGridCurr_[LIN(1, 0, dimX_ + 2)]), dimX_,
			  MPI_DOUBLE, topRank, 0, MPI_COMM_WORLD, &reqs[0]);

		// Bottom.
		MPI_Irecv(&(myGridCurr_[LIN(1, dimY_ + 1, dimX_ + 2)]), dimX_,
			  MPI_DOUBLE, bottomRank, 0, MPI_COMM_WORLD, &reqs[1]);

		// Left.
		MPI_Irecv(leftEdgeIn_, dimY_, MPI_DOUBLE, leftRank, 0,
			  MPI_COMM_WORLD, &reqs[2]);

		// Right.
		MPI_Irecv(rightEdgeIn_, dimY_, MPI_DOUBLE, rightRank, 0,
			  MPI_COMM_WORLD, &reqs[3]);

		/*************** Send ***************/
		// Top.
		MPI_Send(&(myGridCurr_[LIN2(0, 0, dimX_)]), dimX_, MPI_DOUBLE,
			 topRank, 0, MPI_COMM_WORLD);

		// Bottom.
		MPI_Send(&(myGridCurr_[LIN2(0, dimY_ - 1, dimX_)]), dimX_,
			 MPI_DOUBLE, bottomRank, 0, MPI_COMM_WORLD);

		// Left.
		// A bit more annoying, since the data is not stored contiguously.
		for (int i = 0; i < dimY_; ++i)
			leftEdgeOut_[i] = myGridCurr_[LIN2(0, i, dimX_)];
		MPI_Send(leftEdgeOut_, dimY_, MPI_DOUBLE, leftRank, 0,
			 MPI_COMM_WORLD);

		// Right.
		for (int i = 0; i < dimY_; ++i)
			rightEdgeOut_[i] =
				myGridCurr_[LIN2(dimX_ - 1, i, dimX_)];
		MPI_Send(rightEdgeOut_, dimY_, MPI_DOUBLE, rightRank, 0,
			 MPI_COMM_WORLD);
		...
		for (int y = 0; y < dimY_; ++y) {
			...
			myGridCurr_[LIN(dimX_ + 1, y + 1, dimX_ + 2)] =
				rightEdgeIn_[y];
		}
	}
};
