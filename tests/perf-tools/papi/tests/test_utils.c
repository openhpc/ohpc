...
long long **allocate_test_space(int num_tests, int num_events)
{
	...
	for (i = 0; i < num_tests; i++) {
		values[i] = (long long *)malloc((size_t)num_events *
						sizeof(long long));
		if (values[i] == NULL)
			exit(1);
...
int find_nonderived_event(void)
{
	...
	PAPI_event_info_t info;
	int potential_evt_to_add[3] = { PAPI_FP_OPS, PAPI_FP_INS,
					PAPI_TOT_INS };
	int i;

	for (i = 0; i < 3; i++) {
		if (PAPI_query_event(potential_evt_to_add[i]) == PAPI_OK) {
			if (PAPI_get_event_info(potential_evt_to_add[i],
						&info) == PAPI_OK) {
				if ((info.count > 0) &&
...
struct test_events_t test_events[MAX_TEST_EVENTS] = {
	{ MASK_TOT_CYC, PAPI_TOT_CYC }, { MASK_TOT_INS, PAPI_TOT_INS },
	{ MASK_FP_INS, PAPI_FP_INS },	{ MASK_L1_TCM, PAPI_L1_TCM },
	{ MASK_L1_ICM, PAPI_L1_ICM },	{ MASK_L1_DCM, PAPI_L1_DCM },
	{ MASK_L2_TCM, PAPI_L2_TCM },	{ MASK_L2_TCA, PAPI_L2_TCA },
	{ MASK_L2_TCH, PAPI_L2_TCH },	{ MASK_BR_CN, PAPI_BR_CN },
	{ MASK_BR_MSP, PAPI_BR_MSP },	{ MASK_BR_PRC, PAPI_BR_PRC },
	{ MASK_TOT_IIS, PAPI_TOT_IIS }, { MASK_L1_DCR, PAPI_L1_DCR },
	{ MASK_L1_DCW, PAPI_L1_DCW },	{ MASK_L1_DCA, PAPI_L1_DCA },
	...
	*mask = 0;
	...
	if (counters <= 0) {
		test_fail(__FILE__, __LINE__,
			  "Zero Counters Available!  PAPI Won't like this!\n",
			  0);
	}
	...
	for (i = 0; i < 3; i++) {
		if (PAPI_query_event((int)potential_evt_to_add[i][0]) ==
		    PAPI_OK) {
			if (PAPI_get_event_info((int)potential_evt_to_add[i][0],
						&info) == PAPI_OK) {
				if ((info.count > 0) &&
				    ((unsigned)counters > info.count)) {
					*papi_event =
						(int)potential_evt_to_add[i][0];
					*mask = (int)potential_evt_to_add[i][1] |
						MASK_TOT_CYC;
					EventSet = add_test_events(num_events,
								   mask, 1);
					if (*num_events == 2)
						break;
				}
	...
	if (i == 3) {
		test_fail(__FILE__, __LINE__,
			  "Not enough room to add an event!", 0);
	}
	return EventSet;
}
...
#define POTENTIAL_EVENTS 3
	...
	*mask = 0;
	...
	if (i == POTENTIAL_EVENTS) {
		test_fail(__FILE__, __LINE__, "Can't find a non-derived event!",
			  0);
	}
	return EventSet;
}
...
int enum_add_native_events(int *num_events, int **evtcodes, int need_interrupt,
			   int no_software_events, int cidx)
{
	/* query and set up the right event to monitor */
	int EventSet = PAPI_NULL;
	...
	if (need_interrupt) {
		if ((!strcmp(hw_info->model_string, "POWER6")) ||
			...
			test_warn(
				__FILE__, __LINE__,
				"Limiting num_counters because of LIMITED_PMC on Power5 and Power6",
				1);
			counters = 4;
		}
	}
	...
	do {
		...
		if (no_software_events &&
		    (strstr(info.symbol, "PERF_COUNT_SW") ||
		     strstr(info.long_descr, "PERF_COUNT_SW"))) {
			if (!TESTS_QUIET) {
		...
		if (s->cntr_umasks) {
			...
			if (PAPI_enum_cmp_event(&k, PAPI_NTV_ENUM_UMASKS,
						cidx) == PAPI_OK) {
				do {
				...
				} while (PAPI_enum_cmp_event(
						 &k, PAPI_NTV_ENUM_UMASKS,
						 cidx) == PAPI_OK &&
	...
	*num_events = (int)event_found;
	...
	if (!TESTS_QUIET)
		printf("Tried to fill %d counters with events, "
		       "found %d\n",
...
void init_multiplex(void)
{
	...
	if ((strstr(cmpinfo->name, "perfctr.c")) && (hw_info != NULL) &&
		...
		if (retval != PAPI_OK)
			test_fail(__FILE__, __LINE__, "PAPI_set_domain",
				  retval);
	}
