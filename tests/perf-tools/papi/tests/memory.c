...
int main(int argc, char **argv)
{
	...
	const int eventlist[] = { PAPI_L1_DCA,
				  PAPI_L1_DCM,
				  PAPI_L1_DCH,
				  PAPI_L2_DCA,
				  PAPI_L2_DCM,
		...
		for (j = 0; j < 2; j++) {
			...
			if (tmp == PAPI_MH_TYPE_UNIFIED) {
				...
				test_fail(__FILE__, __LINE__,
					  "PAPI_get_hardware_info", PAPI_EBUG);
			}
			...
			tmp = PAPI_MH_CACHE_REPLACEMENT_POLICY(
				L[i].cache[j].type);
	...
	for (i = 0; eventlist[i] != 0; i++) {
		...
		printf("\nEvent: %s\nShort: %s\nLong: %s\n\n", evinfo.symbol,
		       evinfo.short_descr, evinfo.long_descr);
		...
		for (j = 512; j <= 16 * (1024 * 1024); j = j * 2) {
			...
			if ((retval = PAPI_reset(EventSet)) != PAPI_OK)
				test_fail(__FILE__, __LINE__, "PAPI_reset",
					  retval);

			do_misses(1, j);

			if ((retval = PAPI_read(EventSet, &values[0])) !=
			    PAPI_OK)
				test_fail(__FILE__, __LINE__, "PAPI_read",
					  retval);
			if ((retval = PAPI_reset(EventSet)) != PAPI_OK)
				test_fail(__FILE__, __LINE__, "PAPI_reset",
					  retval);
			...
			if ((retval = PAPI_read(EventSet, &values[1])) !=
			    PAPI_OK)
				test_fail(__FILE__, __LINE__, "PAPI_read",
					  retval);
		...
		if ((retval = PAPI_remove_event(EventSet, eventlist[i])) !=
		    PAPI_OK)
			test_fail(__FILE__, __LINE__, "PAPI_remove_event",
				  retval);
	}
