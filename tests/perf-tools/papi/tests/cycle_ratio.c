...
int main(int argc, char **argv)
{
	...
	if (PAPI_OK != retval) {
		test_skip(__FILE__, __LINE__,
			  "PAPI_REF_CYC is not defined on this platform.",
			  PAPI_OK);
	}
	...
	if (retval != PAPI_OK) {
		test_fail(__FILE__, __LINE__, "PAPI_add_events: PAPI_REF_CYC",
			  retval);
	}
...
static void work(int EventSet, int mhz)
{
	...
	cycles_error = 100.0 * ((double)values[1] - (double)elapsed_cyc) /
		       (double)elapsed_cyc;
