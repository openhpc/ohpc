...
int main(int argc, char **argv)
{
  ...
  events[4]=PAPI_BR_UCN;*/
	 ...
	 */
	t2 = (long long)(10000 * 20) *
	     nevents; /* Target: 10000 usec/multiplex, 20 repeats */
	if (t2 > 30e6)
	...
	if (t2 > t1) /* Scale up execution time to match t2 */
		if (t1)
		 ...
		 30e6) /* Make sure execution time is < 30s per repeated test */
		test_skip(__FILE__, __LINE__, "This test takes too much time",
			  retval);
	...
	for (j = 0; j < nevents; j++) {
		...
		if (spread[j] < MPX_TOLERANCE)
			i--;
			 ...
			 MINCOUNTS) /* Neglect inprecise results with low counts */
			i--;
	}
	if (!TESTS_QUIET)
		printf("\n\n");
