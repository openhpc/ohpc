...
int testing_sposv(int argc, char **argv)
{
    ...
    for(u=0; u<2; u++) {
        ...
        /* PLASMA SPOSV */
        PLASMA_sposv(uplo[u], N, NRHS, A2, LDA, B2, LDB);
        ...
        PLASMA_spotrs(uplo[u], N, NRHS, A2, LDA, B2, LDB);
        ...
        /* PLASMA routines */
        PLASMA_spotrf(uplo[u], N, A2, LDA);
        PLASMA_strsm(PlasmaLeft, uplo[u], trans1, PlasmaNonUnit,
                     N, NRHS, 1.0, A2, LDA, B2, LDB);
        PLASMA_strsm(PlasmaLeft, uplo[u], trans2, PlasmaNonUnit,
                     N, NRHS, 1.0, A2, LDA, B2, LDB);
         ...
         */
        {
            float Anorm = PLASMA_slansy( PlasmaOneNorm, uplo[u], N, A1, LDA );
            float Acond;

            info_solution = PLASMA_spocon(uplo[u], N, A2, LDA, Anorm, &Acond);
            if ( info_solution == 0 ) {
                info_solution = check_estimator(uplo[u], N, A1, LDA, A2, Anorm, Acond, eps);
            } else {
                printf(" PLASMA_spocon returned info = %d\n", info_solution );
            }
            if ((info_solution == 0)){
                printf("***************************************************\n");
 ...
 */
static int check_estimator(PLASMA_enum uplo, int N, float *A1, int LDA,
                           ...
                           float eps)
{
    ...
    info_solution = LAPACKE_spocon(LAPACK_COL_MAJOR, lapack_const(uplo), N, A2, LDA, Anorm, &Acond_lapack);
