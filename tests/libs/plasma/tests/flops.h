...
{
	return (side == PlasmaLeft) ? fmuls_gemm(m, m, n) : fmuls_gemm(m, n, n);
}

static double fadds_symm(plasma_enum_t side, double m, double n)
{
	return (side == PlasmaLeft) ? fadds_gemm(m, m, n) : fadds_gemm(m, n, n);
}
...
{
	return (side == PlasmaLeft) ? fmuls_trmm_2(m, n) : fmuls_trmm_2(n, m);
}

static double fadds_trmm(plasma_enum_t side, double m, double n)
{
	return (side == PlasmaLeft) ? fadds_trmm_2(m, n) : fadds_trmm_2(n, m);
}
...
{
	return (m >= n) ? (0.5 * m * n * n - 1. / 6. * n * n * n + 0.5 * m * n -
			   0.5 * n * n + 2. / 3. * n) :
			  (0.5 * n * m * m - 1. / 6. * m * m * m + 0.5 * n * m -
			   0.5 * m * m + 2. / 3. * m);
}
...
{
	return (m > n) ? (m * n * n - 1. / 3. * n * n * n + m * n +
			  0.5 * n * n + 23. / 6. * n) :
			 (n * m * m - 1. / 3. * m * m * m + 2 * n * m -
			  0.5 * m * m + 23. / 6. * m);
}

static double fadds_geqrf(double m, double n)
{
	return (m > n) ? (m * n * n - 1. / 3. * n * n * n + 0.5 * n * n +
			  5. / 6. * n) :
			 (n * m * m - 1. / 3. * m * m * m + n * m -
			  0.5 * m * m + 5. / 6. * m);
}
...
{
	return 2. * m * n * k - (m + n) * k * k + 2. / 3. * k * k * k +
	       2. * n * k - k * k - 5. / 3. * k;
}

static double fadds_ungqr(double m, double n, double k)
{
	return 2. * m * n * k - (m + n) * k * k + 2. / 3. * k * k * k + n * k -
	       m * k + 1. / 3. * k;
}
...
{
	return 2. * m * n * k - (m + n) * k * k + 2. / 3. * k * k * k + m * k +
	       n * k - k * k - 2. / 3. * k;
}
...
{
	return 6. * fmuls_unmql(side, m, n, k) +
	       2. * fadds_unmql(side, m, n, k);
}
...
{
	return 6. * fmuls_unmrq(side, m, n, k) +
	       2. * fadds_unmrq(side, m, n, k);
}
...
{
	return 6. * fmuls_unmlq(side, m, n, k) +
	       2. * fadds_unmlq(side, m, n, k);
}
...
{
	return (m >= n) ? (2. * m * n * n - 2. / 3. * n * n * n + 2. * n * n +
			   20. / 3. * n) :
			  (2. * n * m * m - 2. / 3. * m * m * m + 2. * m * m +
			   20. / 3. * m);
}

static double fadds_gebrd(double m, double n)
{
	return (m >= n) ? (2. * m * n * n - 2. / 3. * n * n * n + n * n -
			   m * n + 5. / 3. * n) :
			  (2. * n * m * m - 2. / 3. * m * m * m + m * m -
			   n * m + 5. / 3. * m);
}
