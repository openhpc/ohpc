...
static int check_solution(int, int , double *, int,
...
int testing_dgesv(int argc, char **argv)
{
    ...
    */
    ...
    LAPACKE_dlarnv_work(1, ISEED, LDAxN, A1);
    for ( i = 0; i < N; i++)
        for (  j = 0; j < N; j++)
            A2[LDA*j+i] = A1[LDA*j+i];

    /* Initialize B1 and B2 */
    LAPACKE_dlarnv_work(1, ISEED, LDBxNRHS, B1);
    for ( i = 0; i < N; i++)
        for ( j = 0; j < NRHS; j++)
    ...
    */
    ...
    /*-------------------------------------------------------------
    *  TESTING DGETRF + DLASWP + DTRSM + DTRSM
    */
    ...
    for (n=1; n<3; n++)
    {
        double Anorm = PLASMA_dlange( norm[n], N, N, A1, LDA );
        double Acond;

        info_solution = PLASMA_dgecon(norm[n], N, A2, LDA, Anorm, &Acond);
        ...
        if ((info_solution == 0)){
            printf("***************************************************\n");
            printf(" ---- TESTING DGETRF + ZGECON(%s) .............. PASSED !\n", normstr[n]);
            printf("***************************************************\n");
        }
        else{
            printf("**************************************************\n");
            printf(" - TESTING DGETRF + ZGECON(%s) ... FAILED !\n", normstr[n]);
            printf("**************************************************\n");
        }
    }
...
static int check_estimator(PLASMA_enum norm, int N, double *A1, int LDA,
                           ...
                           double eps)
{
    ...
    info_solution = LAPACKE_dgecon(LAPACK_COL_MAJOR, lapack_const(norm), N, A2, LDA, Anorm, &Acond_lapack);
