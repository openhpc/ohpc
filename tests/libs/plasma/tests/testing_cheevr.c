...
int testing_cheevr(int argc, char **argv)
{
    ...
    PLASMA_Alloc_Workspace_cheevd(N, N, &T);
    ...
    */
     ...
     * PLASMA CHEEVR
     */
    INFO = PLASMA_cheevr(vec, range, uplo, N, A2, LDA, vl, vu, il, iu, abstol, &nbcomputedeig, W2, T, Q, LDQ);
    ...
    printf("\n");
    printf("------ TESTS FOR PLASMA CHEEVR ROUTINE -------  \n");
    ...
    if ( (info_solution == 0) & (info_ortho == 0) & (info_reduction == 0) ) {
        printf("***************************************************\n");
        printf(" ---- TESTING CHEEVR ...................... PASSED !\n");
        printf("***************************************************\n");
    }
    else {
        printf("************************************************\n");
        printf(" - TESTING CHEEVR ... FAILED !\n");
        printf("************************************************\n");
    }
...
static int check_reduction(int uplo, int N, int bw, PLASMA_Complex32_t *A, float *D, int LDA, PLASMA_Complex32_t *Q, float eps )
{
    ...
    cblas_cgemm(CblasColMajor, CblasNoTrans, CblasConjTrans, N, N, N, CBLAS_SADDR(zone), TEMP, N,  Q, LDA, CBLAS_SADDR(mzone), Residual,     N);
