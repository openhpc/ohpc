...
int testing_dsyevr(int argc, char **argv)
{
    ...
    */
     ...
     * PLASMA DSYEVR
     */
    INFO = PLASMA_dsyevr(vec, range, uplo, N, A2, LDA, vl, vu, il, iu, abstol, &nbcomputedeig, W2, T, Q, LDQ);
    ...
    printf("\n");
    printf("------ TESTS FOR PLASMA DSYEVR ROUTINE -------  \n");
    ...
    if ( (info_solution == 0) & (info_ortho == 0) & (info_reduction == 0) ) {
        printf("***************************************************\n");
        printf(" ---- TESTING DSYEVR ...................... PASSED !\n");
        printf("***************************************************\n");
    }
    else {
        printf("************************************************\n");
        printf(" - TESTING DSYEVR ... FAILED !\n");
        printf("************************************************\n");
    }
...
static int check_reduction(int uplo, int N, int bw, double *A, double *D, int LDA, double *Q, double eps )
{
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasTrans, N, N, N, (zone), TEMP, N,  Q, LDA, (mzone), Residual,     N);
