...
int testing_ssyevr(int argc, char **argv)
{
    ...
    */
     ...
     * PLASMA SSYEVR
     */
    INFO = PLASMA_ssyevr(vec, range, uplo, N, A2, LDA, vl, vu, il, iu, abstol, &nbcomputedeig, W2, T, Q, LDQ);
    ...
    printf("\n");
    printf("------ TESTS FOR PLASMA SSYEVR ROUTINE -------  \n");
    ...
    if ( (info_solution == 0) & (info_ortho == 0) & (info_reduction == 0) ) {
        printf("***************************************************\n");
        printf(" ---- TESTING SSYEVR ...................... PASSED !\n");
        printf("***************************************************\n");
    }
    else {
        printf("************************************************\n");
        printf(" - TESTING SSYEVR ... FAILED !\n");
        printf("************************************************\n");
    }
...
static int check_reduction(int uplo, int N, int bw, float *A, float *D, int LDA, float *Q, float eps )
{
    ...
    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasTrans, N, N, N, (zone), TEMP, N,  Q, LDA, (mzone), Residual,     N);
