...
int testing_dsygvd(int argc, char **argv)
{
    ...
    PLASMA_Alloc_Workspace_dsygvd(N, N, &T);
    ...
    */
    ...
    printf("\n");
    printf("------ TESTS FOR PLASMA DSYGVD ROUTINE -------  \n");
    ...
    for (i=0; i<3; i++) {
        for (u=0; u<2; u++) {
            ...
            /* CALL DSYGVD with itype= 1, 2, 3 and uplo= L,U */ 
            PLASMA_dsygvd(itype[i], vec, uplo[u], N, A2, LDA, B2, LDB, W2, T, Q, LDQ);

            /* CALL LAPACK to compute eigenvalues. */
            memcpy(A1, Ainit, LDAxN*sizeof(double));
            memcpy(B1, Binit, LDBxN*sizeof(double));
            LAPACKE_dsygvd( LAPACK_COL_MAJOR, 
            ...
            if ( (info_ortho == 0) & (info_reduction == 0) & (info_solution == 0)) {
                printf("***************************************************\n");
                printf(" ---- TESTING DSYGVD (%s, %s) ...................... PASSED !\n", itypestr[i], uplostr[u]);
                printf("***************************************************\n");
            }
            else {
                printf("************************************************\n");
                printf(" - TESTING DSYGVD (%s, %s) ... FAILED !\n", itypestr[i], uplostr[u]);
                printf("************************************************\n");
            } 
        }
    }
