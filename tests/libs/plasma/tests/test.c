...
static param_desc_t ParamDesc[] = {
	...
	{ "success", "Status", 6, false, "success indicator" },

	{ "error", "Error", 9, false, "numerical error" },

	{ "ortho", "Orth. error", 9, false, "orthogonality error" },

	{ "time", "Time", 9, false, "time to solution" },

	{ "gflops", "Gflop/s", 9, false, "GFLOPS rate" },

	{ "itersv", "IterSv", 9, false, "iterations to solution" },
	...
	{ "--test=[y|n]", "test", 0, false, "test the solution [default: y]" },

	{ "--tol=", "tol", 0, false, "tolerance [default: 50.0]" },
	...
	{ "--trans=[n|t|c]", "trans", 6, true, "transposition [default: n]" },
	...
	{ "--dim=", "Dimensions", 6, true,
	  ...
	  "M, N, K can each be a single value or a range.\n" INDENT
	  "N and K are optional; if not given, N=M and K=N.\n" INDENT
	  "Ex: --dim=100:300:100x64 is 100x64x64, 200x64x64, 300x64x64." },

	{ "--kl=", "kl", 6, true, "Lower bandwidth [default: 200]" },

	{ "--ku=", "ku", 6, true, "Upper bandwidth [default: 200]" },

	{ "--nrhs=", "nrhs", 6, true,
	  "NHRS dimension (number of columns) [default: 1000]" },

	{ "--nb=", "nb", 4, true, "NB size of tile (NB by NB) [default: 256]" },

	{ "--ib=", "ib", 4, true, "IB inner blocking size [default: 64]" },

	{ "--alpha=", "alpha", 14, true, "scalar alpha" },

	{ "--beta=", "beta", 14, true, "scalar beta" },

	{ "--pada=", "padA", 4, true, "padding added to lda [default: 0]" },

	{ "--padb=", "padB", 4, true, "padding added to ldb [default: 0]" },

	{ "--padc=", "padC", 4, true, "padding added to ldc [default: 0]" },
 ...
 *
 ******************************************************************************/
void print_routine_usage(const char *program_name, const char *name,
			 param_value_t pval[])
{
	printf("Usage:\n"
	       "\t%s %s [-h|--help]\n"
	       ...
	       "Options:\n"
	       "\t%*sshow this screen\n",
	       program_name, name, program_name, name, DescriptionIndent,
	       "-h --help");
...
{
	printf("\t%*s%s\n", DescriptionIndent, ParamDesc[label].arg,
	...
	for (int i = 0; i < PARAM_SIZEOF; ++i) {
		if (pval[i].used) {
			...
			case PARAM_BETA:
				...
				printf("  %5.2f + %5.2fi", creal(pval[i].z),
				       cimag(pval[i].z));
				break;
...
void param_init(param_t param[])
{
	for (int i = 0; i < PARAM_SIZEOF; i++) {
		...
		param[i].num = 0;
		param[i].pos = 0;
		param[i].val = (param_value_t *)malloc(InitValArraySize *
...
void param_read(int argc, char **argv, param_t param[])
{
	...
	for (int i = 2; i < argc && argv[i]; i++) {
		...
		//--------------------------------------------------
			...
			err = param_scan_int(strchr(argv[i], '=') + 1,
					     &param[PARAM_ITER]);

		else if (param_starts_with(argv[i], "--dim=")) {
			bool outer = param[PARAM_DIM_OUTER].val[0].c == 'y';
			err = param_scan_int3(strchr(argv[i], '=') + 1,
					      &param[PARAM_DIM], outer);
		}
		...
		//--------------------------------------------------
			...
			err = param_scan_double(strchr(argv[i], '=') + 1,
						&param[PARAM_TOL]);
...
int scan_irange(const char **strp, int *start, int *end, int *step)
{
	...
	cnt = sscanf(*strp, "%d %n: %d %n: %d %n", start, &bytes1, end, &bytes2,
		     step, &bytes3);
	if (cnt == 3) {
		if (*start == *end)
			*step = 0;
...
{
	int bytes1, bytes2, bytes3, cnt;
	cnt = sscanf(*strp, "%lf %n: %lf %n: %lf %n", start, &bytes1, end,
		     &bytes2, step, &bytes3);
	if (cnt == 3) {
		if (*start == *end)
			*step = 0;
...
int param_scan_int3(const char *str, param_t *param, bool outer)
{
	...
	while (true) {
		...
		if (len > 0) {
			...
			if (len > 0) {
				str += len;
				if (scan_irange(&str, &k_start, &k_end,
						&k_step) != 0)
					return 1;
			} else {
				k_start = n_start;
...
int param_scan_double(const char *str, param_t *param)
{
	...
	while (true) {
		...
		if (start == end) {
			...
			for (double d = start; d <= end + step / 10.;
			     d += step) {
	...
	if (param->is_list) {
		...
		if (param->num == param->size) {
			...
			param->val = (param_value_t *)realloc(
				param->val,
				param->size * sizeof(param_value_t));
