...
int main(int argc, char *argv[])
{
      ...
      We have one variable in each part. */
	{
		...
		if (myid == 0) {
			/* Add the first box to the grid in part 0 */
			{
				...
				part = 0;
				HYPRE_SStructGridSetExtents(grid, part, ilower,
							    iupper);
			}
	...
	/* 2. Define the discretization stencils */
	{
		...
		/* the 5-pt stencil in 2D */
		{
			...
			HYPRE_SStructStencilCreate(ndim, stencil_size,
						   &stencil_5pt);

			for (entry = 0; entry < 5; entry++)
				HYPRE_SStructStencilSetEntry(stencil_5pt, entry,
							     offsets[entry],
							     var);
		}
		...
		/* the 9-pt stencil in 2D */
		{
			...
			for (entry = 0; entry < stencil_size; entry++)
				HYPRE_SStructStencilSetEntry(stencil_9pt, entry,
							     offsets[entry],
							     var);
		}
	}
	...
	/* 4. Set up a SStruct Matrix */
	{
		...
		if (myid == 0) {
            ...
            grid points later) */
			{
				...
				int nentries = 5;
				int nvalues =
					30; /* 6 grid points, each with 5 stencil entries */
				double values[30];

				int stencil_indices[5];
				for (j = 0; j < nentries;
				     j++) /* label the stencil indices -
            ...
            over the gridpoints in my second box */
			{
				...
				int nentries = 9;
				int nvalues =
					108; /* 12 grid points, each with 5 stencil entries */
				double values[108];
            ...
            over the gridpoints in my box */
			{
				...
				int nentries = 5;
				int nvalues =
					80; /* 16 grid points, each with 5 stencil entries */
				double values[80];
		...
		if (myid == 0) {
			int nentries = 6;
			int nvalues =
				24; /* 4 grid points, each with 6 stencil entries */
			double values[24];
		...
		if (myid == 0) {
...

			{
				...
				HYPRE_SStructMatrixSetBoxValues(A, part, ilower,
								iupper, var, 1,
								stencil_indices,
								values);
			}
		...
		if (myid == 0) {
			/* Set the vector coefficients over the gridpoints in my first box */
			{
				...
				HYPRE_SStructVectorSetBoxValues(
					b, part, ilower, iupper, var, values);

				for (i = 0; i < nvalues; i++)
					values[i] = 0.0;
				HYPRE_SStructVectorSetBoxValues(
					x, part, ilower, iupper, var, values);
			}
