...
void GLVis_PrintLocalRhombusMesh(const char *meshfile_prefix, int n, int myid,
				 double gamma)
{
	FILE *file;
	char meshfile[255];
...
void GLVis_PrintLocalCubicMesh(const char *meshfile_prefix, int nx, int ny,
			       int nz, double h, double x0, double y0,
			       double z0, int myid)
{
	FILE *file;
	char meshfile[255];
...
void GLVis_PrintSStructGrid(HYPRE_SStructGrid grid, const char *meshfile_prefix,
			    int myid, double *trans, double *origin)
{
	FILE *file;
	char meshfile[255];
...

	{
		...
		for (p = 0; p < nparts; p++) {
			...
			for (b = 0; b < hypre_BoxArraySize(boxes); b++) {
				box = hypre_BoxArrayBox(boxes, b);
				for (k = hypre_BoxIMinD(box, 2);
				     k <= hypre_BoxIMaxD(box, 2); k++)
					for (j = hypre_BoxIMinD(box, 1);
					     j <= hypre_BoxIMaxD(box, 1); j++)
						for (i = hypre_BoxIMinD(box, 0);
						     i <=
...
void GLVis_PrintSStructVector(HYPRE_SStructVector sol, int var,
			      const char *solfile_prefix, int myid)
{
	FILE *file;
	char solfile[255];
	...
	for (p = 0; p < nparts; p++) {
		...
		for (b = 0; b < hypre_BoxArraySize(boxes); b++) {
			...
			if (dim == 2)
				values = (double *)malloc((ni + var_off) *
							  (nj + var_off) *
							  sizeof(double));
			else {
				values = (double *)malloc(
					(ni + var_off) * (nj + var_off) *
					(nk + var_off) * sizeof(double));
				ilower[2] = hypre_BoxIMinD(box, 2) - var_off;
				iupper[2] = hypre_BoxIMaxD(box, 2);
			}

			HYPRE_SStructVectorGetBoxValues(sol, p, ilower, iupper,
							var, values);
...
/* Save a GLVis mesh file with the given prefix corresponding to the input
   ...
   Cartesian grid. */
void GLVis_PrintStructGrid(HYPRE_StructGrid Grid, const char *meshfile_prefix,
			   int myid, double *trans, double *origin)
{
	FILE *file;
	char meshfile[255];
...
void GLVis_PrintStructVector(HYPRE_StructVector sol, const char *solfile_prefix,
			     int myid)
{
	FILE *file;
	char solfile[255];
	...
	for (b = 0; b < hypre_BoxArraySize(boxes); b++) {
		...
		if (dim == 2)
			...
			values =
				(double *)malloc(ni * nj * nk * sizeof(double));
