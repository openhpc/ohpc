...
int main(int argc, char *argv[])
{
	...
	/* 3. Set up a Struct Matrix */
	{
		...
		if (myid == 0) {
			...
			int stencil_indices[5] = {
				0, 1, 2, 3, 4
			}; /* labels for the stencil entries -
		...
		if (myid == 0) {
				...
				values[i] = 0.0;
			{
				...
				HYPRE_StructMatrixSetBoxValues(A, ilower,
							       iupper, 1,
							       stencil_indices,
							       values);
			}
      ...
      corresponding to its boxes. */
	{
		...
		if (myid == 0) {
			...
			for (i = 0; i < 6; i++)
				values[i] = 1.0;
			HYPRE_StructVectorSetBoxValues(b, ilower, iupper,
						       values);

			for (i = 0; i < 6; i++)
				values[i] = 0.0;
			HYPRE_StructVectorSetBoxValues(x, ilower, iupper,
						       values);
		} else if (myid == 1) {
			int ilower[2] = { 0, 1 }, iupper[2] = { 2, 4 };
      ...
      of all of the options.) */
	{
		...
		/* Set some parameters */
		HYPRE_StructPCGSetTol(solver,
				      1.0e-06); /* convergence tolerance */
		HYPRE_StructPCGSetPrintLevel(solver,
					     2); /* amount of info. printed */
