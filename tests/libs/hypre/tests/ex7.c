...
int main(int argc, char *argv[])
{
	...
	/* 1. Set up a 2D grid */
	{
		...
		/* Set the variable type for each part */
		{
			...
			for (i = 0; i < nparts; i++)
				HYPRE_SStructGridSetVariables(grid, i, nvars,
							      vartypes);
		}
	...
	/* 2. Define the discretization stencil */
	{
		...
		if (sym == 0) {
			...
			for (i = 0; i < 5; i++)
				HYPRE_SStructStencilSetEntry(stencil, i,
							     offsets[i], var);
	...
	/* 4. Set up SStruct Vectors for b and x */
	{
		...
		HYPRE_SStructVectorSetBoxValues(b, part, ilower, iupper, var,
						values);

		/* Set x = 0 */
		for (i = 0; i < (n * n); i++)
			values[i] = 0.0;
		HYPRE_SStructVectorSetBoxValues(x, part, ilower, iupper, var,
						values);
         ...
         stencil and the corresponding rhs entries. */
	{
		...
		if (pj == 0) {
			...
			HYPRE_SStructVectorSetBoxValues(
				b, part, bc_ilower, bc_iupper, var, bvalues);
		}
		...
		if (pj == N - 1) {
			...
			HYPRE_SStructMatrixSetBoxValues(A, part, bc_ilower,
							bc_iupper, var, 1,
							stencil_indices,
							bvalues);
	...
	/* 6. Set up and use a solver */
	{
		...
		if (solver_id == 0) /* SMG */
		{
			...
			HYPRE_StructSMGGetNumIterations(solver,
							&num_iterations);
		...
		if (solver_id == 1) /* PFMG */
		{
			...
			/* Get info and release memory */
			HYPRE_StructPFMGGetNumIterations(solver,
							 &num_iterations);
		...
		if ((solver_id > 9) && (solver_id < 20)) {
			...
			if (solver_id == 10) {
				...
				HYPRE_StructPFMGSetNumPostRelax(precond,
								n_post);
				...
				HYPRE_StructJacobiCreate(MPI_COMM_WORLD,
							 &precond);
				...
				HYPRE_StructPCGSetPrecond(
					solver, HYPRE_StructJacobiSolve,
					HYPRE_StructJacobiSetup, precond);
			}
				...
				precond = NULL;
				HYPRE_StructPCGSetPrecond(
					solver, HYPRE_StructDiagScale,
					HYPRE_StructDiagScaleSetup, precond);
			}
			...
			/* Get info and release memory */
			HYPRE_StructPCGGetNumIterations(solver,
							&num_iterations);
		...
		if ((solver_id > 29) && (solver_id < 40)) {
			...
			/* Get info and release memory */
			HYPRE_StructGMRESGetNumIterations(solver,
							  &num_iterations);
	...
	if (vis) {
		...
		HYPRE_SStructVectorGetBoxValues(x, part, ilower, iupper, var,
						values);
