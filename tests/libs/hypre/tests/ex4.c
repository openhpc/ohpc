...
int main(int argc, char *argv[])
{
         ...
         stencil and the corresponding rhs entries. */
	{
		...
		if (pj == 0) {
			...
			/* Modify the matrix */
			HYPRE_StructMatrixSetBoxValues(A, bc_ilower, bc_iupper,
						       nentries,
						       stencil_indices, values);
			...
			HYPRE_StructVectorSetBoxValues(b, bc_ilower, bc_iupper,
						       bvalues);
		}
		...
		if (pj == 0) {
			...
			if (sym == 0)
				HYPRE_StructMatrixSetBoxValues(A, bc_ilower,
							       bc_iupper, 1,
							       stencil_indices,
							       bvalues);
            ...
            at these nodes) */
			HYPRE_StructVectorAddToBoxValues(b, bc_ilower,
							 bc_iupper, bvalues);
		}
	...
	if (solver_id == 0) /* SMG */
	{
		...
		HYPRE_StructSMGGetFinalRelativeResidualNorm(solver,
							    &final_res_norm);
	...
	if ((solver_id > 9) && (solver_id < 20)) {
		...
		if (solver_id == 10) {
			...
			precond = NULL;
			HYPRE_StructPCGSetPrecond(solver, HYPRE_StructDiagScale,
		...
		HYPRE_StructPCGGetFinalRelativeResidualNorm(solver,
							    &final_res_norm);
