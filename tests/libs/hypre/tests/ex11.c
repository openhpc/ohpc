...
int main(int argc, char *argv[])
{
   ...
   */
	...
	/* AMG preconditioner */
	{
		...
		HYPRE_BoomerAMGSetNumSweeps(precond,
					    2); /* 2 sweeps of smoothing */
		HYPRE_BoomerAMGSetTol(precond, 0.0); /* conv. tolerance zero */
		HYPRE_BoomerAMGSetMaxIter(precond,
	...
	/* LOBPCG eigensolver */
	{
		...
		int pcgMode =
			1; /* use rhs as initial guess for inner pcg iterations */
		...
		eigenvectors = mv_MultiVectorCreateFromSampleVector(
			interpreter, blockSize, par_x);
		mv_MultiVectorSetRandom(eigenvectors, lobpcgSeed);

		/* eigenvectors - get a pointer */
		{
			mv_TempMultiVector *tmp =
				(mv_TempMultiVector *)mv_MultiVectorGetData(
					eigenvectors);
		...
		HYPRE_LOBPCGSolve(lobpcg_solver, constraints, eigenvectors,
				  eigenvalues);
