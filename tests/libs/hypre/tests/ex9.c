...
int main(int argc, char *argv[])
{
	...
	/* 2. Define the discretization stencils */
	{
		...
		/* Stencil object for variable u (labeled as variable 0) */
		{
			...
			for (entry = 0; entry < stencil_size - 1; entry++)
				HYPRE_SStructStencilSetEntry(
					stencil_u, entry, offsets[entry], var);

			/* The last entry is for the u-v connection */
			var = 1; /* connect to variable 1 */
			entry = 5;
			HYPRE_SStructStencilSetEntry(stencil_u, entry,
						     offsets[entry], var);
		}
		...
		/* Stencil object for variable v  (variable 1) */
		{
			...
			for (entry = 0; entry < stencil_size; entry++)
				HYPRE_SStructStencilSetEntry(
					stencil_v, entry, offsets[entry], var);
		}
	}
         ...
         to zero.*/
	{
		...
		int bc_iupper[2];
		int nentries = 1;
		int nvalues = nentries *
			      n; /*  number of stencil entries times the length
      ...
      (Solver options can be found in the Reference Manual.) */
	{
		...
		if (solver_id == 0) /* GMRES with SysPFMG - the default*/
		{
			...
			HYPRE_SStructGMRESSetPrintLevel(solver,
							2); /* print each GMRES
                                                         iteration */
		...
		} else if (solver_id == 2) /* GMRES with AMG */
		{
			...
			HYPRE_BoomerAMGSetPrintFileName(par_precond,
							"ex9.out.log");
		...
		} else if (solver_id == 3) /* AMG */
		{
			...
			HYPRE_BoomerAMGSetPrintFileName(par_solver,
							"ex9.out.log");
		...
		if (vis) {
			...
			int nvalues = n * n;
			double *values =
				(double *)calloc(nvalues, sizeof(double));
			...
			if (myid == 0)
				GLVis_PrintGlobalSquareMesh("vis/ex9.mesh",
							    N * n - 1);
		}
