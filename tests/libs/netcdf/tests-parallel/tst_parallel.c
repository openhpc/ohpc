...
int main(int argc, char **argv)
{
...
#ifdef USE_MPE
	int s_init, e_init, s_define, e_define, s_write, e_write, s_close,
		e_close;
#endif /* USE_MPE */
	...
	if ((res = nc_create_par(file_name, NC_NETCDF4 | NC_MPIIO, comm, info,
				 &ncid)))
		ERR;

	/* Create three dimensions. */
	if (nc_def_dim(ncid, "d1", DIMSIZE, dimids))
		ERR;
	if (nc_def_dim(ncid, "d2", DIMSIZE, &dimids[1]))
		ERR;
	if (nc_def_dim(ncid, "d3", NUM_SLABS, &dimids[2]))
		ERR;
	...
	if (nc_var_par_access(ncid, v1id, NC_COLLECTIVE))
		ERR;
	...
	for (start[2] = 0; start[2] < NUM_SLABS; start[2]++) {
		...
		if (nc_put_vara_int(ncid, v1id, start, count, slab_data))
			ERR;
#ifdef USE_MPE
