	...
	char *path; /* name of writable netcdf file to open */
{
	...
	if (iv < test.nvars) { /* iv is varid of variable with dimensions */
		...
		if (ncvarget(cdfid, iv, hc.cor, hc.edg, hc.vals) != -1) {
			error("%s: ncvarget should fail for negative corner",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (ncvarget(cdfid, iv, hc.cor, hc.edg, hc.vals) != -1) {
			error("%s: ncvarget should fail for negative edge",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
...

		{
			...
			if (ncvarget(cdfid, iv, hc.cor, hc.edg, hc.vals) !=
			    -1) {
				error("%s: ncvarget should fail for too-high coordinate",
				      pname);
				ncclose(cdfid);
				return ++nerrs;
			}
			...
			if (ncvarget(cdfid, iv, hc.cor, hc.edg, hc.vals) !=
			    -1) {
				error("%s: ncvarget should fail for too-high edge",
				      pname);
				ncclose(cdfid);
				return ++nerrs;
			}
	...
	if (ncvarget(cdfid, -1, hc.cor, hc.edg, hc.vals) != -1) {
		error("%s: ncvarget should fail for bad variable handle",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
