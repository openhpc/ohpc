 ...
 * error.
 */
static int numrecvars(ncid, recvarids)
int ncid;
int *recvarids;
{
	int ndims, iv, nvars;
 ...
 */
static long ncrecsize(ncid, vid)
int ncid;
int vid;
{
	int recdimid;
 ...
 * errors better.
 */
static int recinq(ncid, nrecvars, recvarids, recsizes)
int ncid;
int *nrecvars;
int *recvarids;
long *recsizes;
 ...
 */
int test_ncrecinq(path) const char *path; /* name of netcdf file to open */
{
	int nerrs = 0;
 ...
 */
static int dimsizes(ncid, varid, sizes)
int ncid;
int varid;
long *sizes;
{
 ...
 * better.
 */
static int recput(ncid, recnum, datap)
int ncid;
long recnum;
void **datap;
{
	...
	for (iv = 0; iv < nrvars; iv++) {
		if (datap[iv] != 0) {
			...
			if (ncvarput(ncid, rvids[iv], start, edges,
				     datap[iv]) == -1)
				return -1;
		}
	}
 ...
 * better.
 */
static int recget(ncid, recnum, datap)
int ncid;
long recnum;
void **datap;
{
	...
	for (iv = 0; iv < nrvars; iv++) {
		if (datap[iv] != 0) {
			(void)dimsizes(ncid, rvids[iv], edges);
			edges[0] = 1; /* only 1 record's worth */
			if (ncvarget(ncid, rvids[iv], start, edges,
				     datap[iv]) == -1)
				return -1;
		}
	}
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	for (iv = 0; iv < nrvars; iv++) {
		...
		if (ncvarinq(ncid, rvarids[iv], 0, &vartype[iv], 0, 0, 0) ==
		    -1) {
	...
	for (iv = 0; iv < nrvars; iv++) {
		if (val_cmp(vartype[iv], recsize[iv], datap[iv], datar[iv]) !=
		    0) {
	...
	if (nrvars > 0) {
		...
		if (recget(ncid, recnum, datap) == -1) {
			error("%s: recget failed after partial record put",
			      pname);
			nerrs++;
		}
		for (iv = 0; iv < nrvars; iv++) {
			if (val_cmp(vartype[iv], recsize[iv], datap[iv],
				    datar[iv]) != 0) {
				error("%s: bad values written by partial recput",
				      pname);
				nerrs++;
			}
		}
 ...
 *    try with bad netCDF handle, check error
 */
int test_ncrecget(path) const char *path; /* name of netcdf file to open */
{
	int nerrs = 0;
	...
	if (nrvars > 0) {
		...
		for (iv = 0; iv < nrvars; iv++) {
			if (val_cmp(vartype[iv], recsize[iv], datap[iv],
				    datar[iv]) != 0) {
				error("%s: bad values read by partial recget",
				      pname);
				nerrs++;
			}
		}
