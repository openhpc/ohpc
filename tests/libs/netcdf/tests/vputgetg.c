...
#define INTNPTS(extnpts, idim) (1 + (extnpts - 1) / STRIDE(idim))
		...
		for (id = test.vars[iv].ndims - 1; id >= 0;
		     --id) { /* set cubes */
	     ...
	     * from "upper-right" corner. */
			...
			hc[2].npts[id] =
				INTNPTS(max(EXTNPTS(iv, id) / 3, 1), id);
		...
		for (ie = 0; ie < ne; ie++) { /* for each test */
			int nelms =
				(int)nel[ie] * nctypelen(test.vars[iv].type) +
				8;
			...
			if (ncvarputg(cdfid, iv, hc[ie].cor, hc[ie].npts,
				...
				/* keep test in sync */
				if (ncvargetg(cdfid, iv, hc[ie].cor,
					      hc[ie].npts, hc[ie].strd,
					      hc[ie].imap,
					      (char *)tmp.vals +
						      hc[ie].offset) == -1) {
