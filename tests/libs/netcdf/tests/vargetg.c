...
#define TEST_FAILS(varid)                                               \
	(ncvarputg(cdfid, varid, hc.cor, hc.edg, strides, (long *)NULL, \
		...
		if (TEST_FAILS(iv)) {
			error("%s: ncvargetg should fail for negative corner",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (TEST_FAILS(iv)) {
			error("%s: ncvargetg should fail for negative edge",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (TEST_FAILS(iv)) {
			error("%s: ncvargetg should fail for too-high coordinate",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (TEST_FAILS(iv)) {
			error("%s: ncvargetg should fail for too-high edge",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (TEST_FAILS(iv)) {
			error("%s: ncvargetg should fail for non-positive stride",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
	...
	if (TEST_FAILS(-1)) {
		error("%s: ncvargetg should fail for bad variable handle",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	if (TEST_FAILS(0)) {
		error("%s: ncvargetg failed to report bad netcdf handle",
		      pname);
		nerrs++;
	}
	free(hc.vals);
