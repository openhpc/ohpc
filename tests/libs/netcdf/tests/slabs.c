...
int cdfid; /* handle of netcdf open and in data mode */
{
	...
	static struct cdfdim dims[NDIMS] = {
		{ "w", WSIZE }, { "x", XSIZE }, { "y", YSIZE }, { "z", ZSIZE }
	};
	...
	for (idim = 0; idim < NDIMS; idim++) {
		dimids[idim] =
			ncdimdef(cdfid, dims[idim].name, dims[idim].size);
		if (dimids[idim] == -1) {
	...
	for (iv = 0; iv < NVARS; iv++) {
		...
		varid[iv] = ncvardef(cdfid, va[iv].name, va[iv].type,
				     va[iv].ndims, va[iv].dims);
		if (varid[iv] == -1) {
	...
	for (iv = 0; iv < NVARS; iv++) { /* test each type of variable */
		...
		v = emalloc(WSIZE * XSIZE * YSIZE * ZSIZE *
		...
		add_data(&test, varid[iv], corner,
			 edge); /* keep test in sync */
		/*
	 ...
	 */
		...
		for (idim = 0; idim < NDIMS; idim++) {
			...
			if (ncvarget(cdfid, varid[iv], corner, edge,
				     (void *)v) == -1) {
			...
			for (ii = (int)corner[idim]; ii <= edge[idim]; ii++) {
				point[idim] = ii;
				/* if (v[ii-1] != VF(point)) */
				if (val_diff(va[iv].type, v, ii - 1,
					     VF(point))) {
					error("%s: ncvarget got wrong value for vector",
					      pname);
					nerrs++;
				}
			}
		...
		for (idim = 0; idim < NDIMS; idim++) {
			for (jdim = idim + 1; jdim < NDIMS; jdim++) {
				for (kdim = 0; kdim < NDIMS;
				     kdim++) { /* reset corners and edges */
					corner[kdim] = dims[kdim].size / 2;
					edge[kdim] = 1;
					point[kdim] = corner[kdim];
				}
				corner[idim] =
					1; /* interior plane along dimensions idim jdim */
				corner[jdim] = 1;
				edge[idim] = dims[idim].size - 2;
				edge[jdim] = dims[jdim].size - 2;
				if (ncvarget(cdfid, varid[iv], corner, edge,
					     (void *)v) == -1) {
					error("%s: ncvarget of plane failed",
					      pname);
					nerrs++;
				}
				for (ii = (int)corner[idim]; ii <= edge[idim];
				     ii++) {
					for (jj = (int)corner[jdim];
					     jj <= edge[jdim]; jj++) {
						point[idim] = ii;
						point[jdim] = jj;
						...
						if (val_diff(
							    ...
							    (ii -
							     1) * (int)edge[jdim] +
								    jj - 1,
							    VF(point))) {
							error("%s: ncvarget got wrong value in plane",
							      pname);
		...
		for (idim = 0; idim < NDIMS; idim++) {
			for (jdim = idim + 1; jdim < NDIMS; jdim++) {
				for (kdim = jdim + 1; kdim < NDIMS; kdim++) {
					for (ldim = 0; ldim < NDIMS;
					     ldim++) { /* reset corners, edges */
						corner[ldim] =
							dims[ldim].size / 2;
						edge[ldim] = 1;
					...
					corner[idim] =
						1; /* intr. cube along idim jdim kdim */
					corner[jdim] = 1;
					corner[kdim] = 1;
					edge[idim] = dims[idim].size - 2;
					edge[jdim] = dims[jdim].size - 2;
					edge[kdim] = dims[kdim].size - 2;
					if (ncvarget(cdfid, varid[iv], corner,
						     edge, (void *)v) == -1) {
						error("%s: ncvarget of cube failed",
						      pname);
						nerrs++;
					}
					for (ii = (int)corner[idim];
					     ii <= edge[idim]; ii++) {
						for (jj = (int)corner[jdim];
						     jj <= edge[jdim]; jj++) {
							for (kk = (int)
								...
								if (val_diff(
									    ...
									    ((ii -
									     ...
									     1) * (int)edge[kdim] +
										    ...
										    1,
									    VF(point))) {
									error("%s: ncvarget got wrong value in cube",
									      pname);
