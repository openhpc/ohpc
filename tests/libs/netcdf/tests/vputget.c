...
int cdfid; /* handle of netcdf open and in data mode */
{
	...
	for (iv = 0; iv < test.nvars; iv++) { /* for each var in netcdf */
		...
		for (ie = 0; ie < ne; ie++) { /* for each of ne points */
			...
			hc[ie].vals = emalloc(
				nel[ie] * nctypelen(test.vars[iv].type) + 8);
			tmp.vals = emalloc(
				nel[ie] * nctypelen(test.vars[iv].type) + 8);
			...
			if (ncvarput(cdfid, iv, hc[ie].cor, hc[ie].edg,
				     hc[ie].vals) == -1) {
				...
				add_data(&test, iv, hc[ie].cor,
					 hc[ie].edg); /* keep test in sync */
				if (ncvarget(cdfid, iv, hc[ie].cor, hc[ie].edg,
					     tmp.vals) == -1) {
