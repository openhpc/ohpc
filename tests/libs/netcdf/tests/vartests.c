	...
	char *path; /* name of writable netcdf file to open */
{
	...
	for (id = 0; id < xx.ndims; id++)
		xx.dims[id] = id;
	if ((varid = ncvardef(cdfid, xx.name, xx.type, xx.ndims, xx.dims)) ==
	    -1) {
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	for (varid = 0; varid < test.nvars; varid++) { /* loop on all var ids */
		if (ncvarinq(cdfid, varid, var.name, &var.type, &var.ndims,
			     var.dims, &var.natts) == -1) {
	...
	if (ncvarinq(cdfid, -1, var.name, &var.type, &var.ndims, var.dims,
		     &var.natts) != -1 ||
	    ncvarinq(cdfid, test.nvars, var.name, &var.type, &var.ndims,
		     var.dims, &var.natts) != -1) {
	...
	if (test.nvars >= 1) { /* if any variables have been defined */
		if (ncvarinq(cdfid, varid, var.name, &var.type, &var.ndims,
			     var.dims, &var.natts) != -1) {
			error("%s: ncvarinq failed to report bad netcdf handle ",
			      pname);
			nerrs++;
		}
	}
 ...
 */
static int test_varputget1(cdfid)
int cdfid; /* handle of netcdf open and in data mode */
{
	...
	for (iv = 0; iv < test.nvars; iv++) { /* for each var in netcdf */
		...
		for (ie = 0; ie < ne; ie++) { /* for each of ne points */
			switch (test.vars[iv]
					.type) { /* get values of right type to put */
			case NC_BYTE:
			case NC_CHAR:
			...
			case NC_DOUBLE:
				...
				break;
			default:
				error("%s: bad type, test program error",
				      pname);
			}
			...
			add_data(&test, iv, elm[ie].coords,
				 edges); /* keep test in sync */
			...
			switch (test.vars[iv]
					.type) { /* compare values of right type */
			case NC_BYTE:
			case NC_CHAR:
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	if (iv < test.nvars) { /* iv is varid of variable with dimensions */
		...
		if (ncvarput1(cdfid, iv, elm.coords, (void *)&elm.val) != -1) {
			error("%s: ncvarput1 should fail for negative coordinate",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		elm.coords[test.vars[iv].ndims / 2] =
			test.dims[test.vars[iv].dims[test.vars[iv].ndims / 2]]
				.size;
		if (ncvarput1(cdfid, iv, elm.coords, (void *)&elm.val) != -1) {
			error("%s: ncvarput1 should fail for too-high coordinate",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
	...
	if (ncvarput1(cdfid, -1, elm.coords, (void *)&elm.val) != -1 ||
		...
		error("%s: ncvarput1 should fail for bad variable handle",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	if (test.nvars > 0)
		if (ncvarput1(cdfid, 0, elm.coords, (void *)&elm.val) != -1) {
			error("%s: ncvarput1 should fail in define mode",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
	...
	if (ncvarput1(cdfid, 0, elm.coords, (void *)&elm.val) != -1) {
		error("%s: ncvarput1 failed to report bad netcdf handle",
		      pname);
		nerrs++;
	}
	if (nerrs > 0)
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	if (iv < test.nvars) { /* iv is varid of variable with dimensions */
		...
		if (ncvarget1(cdfid, iv, elm.coords, (void *)&elm.val) != -1) {
			error("%s: ncvarget1 should fail for negative coordinate",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		elm.coords[test.vars[iv].ndims / 2] =
			test.dims[test.vars[iv].dims[test.vars[iv].ndims / 2]]
				.size;
		if (ncvarget1(cdfid, iv, elm.coords, (void *)&elm.val) != -1) {
			error("%s: ncvarget1 should fail for too-high coordinate",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
	...
	if (ncvarget1(cdfid, -1, elm.coords, (void *)&elm.val) != -1 ||
		...
		error("%s: ncvarget1 should fail for bad variable handle",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	/* try with bad netCDF handle, should fail */
	if (ncvarget1(cdfid, 0, elm.coords, (void *)&elm.val) != -1) {
		error("%s: ncvarget1 failed to report bad netcdf handle",
		      pname);
		nerrs++;
	}
	if (nerrs > 0)
 ...
 */
int test_ncvarrename(path) const
	char *path; /* name of writable netcdf file to open */
{
	int nerrs = 0;
	...
	for (id = 0; id < yy.ndims; id++)
		yy.dims[id] = id;
	if ((yy_id = ncvardef(cdfid, yy.name, yy.type, yy.ndims, yy.dims)) ==
	    -1) {
		error("%s: ncvardef failed", pname);
		ncclose(cdfid);
		return ++nerrs;
	...
	var.dims = (int *)emalloc(sizeof(int) * MAX_VAR_DIMS);
	var.name = (char *)emalloc(MAX_NC_NAME);
	if (ncvarinq(cdfid, yy_id, var.name, &var.type, &var.ndims, var.dims,
		     &var.natts) == -1) {
	...
	if (strcmp(var.name, newname) != 0) {
		error("%s: ncvarrename changed name to %s instead of %s", pname,
		      var.name, newname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	if (ncvarrename(cdfid, yy_id, zz.name) != -1) {
		error("%s: ncvarrename should have failed with used name",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	if (ncvarrename(cdfid, yy_id, "a_longer_name") != -1) {
		error("%s: ncvarrename to longer should fail in data mode",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	if (ncvarrename(cdfid, yy_id, shortname) == -1) {
		error("%s: ncvarrename to shorter should succeed in data mode",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	(void)strcpy(test.vars[yy_id].name,
		     shortname); /* keep test consistent */
	/* check new name with ncvarid, ncvarinq */
	...
	if (ncvarrename(cdfid, 0, var.name) != -1) {
		error("%s: ncvarrename failed to report bad netcdf handle ",
		      pname);
		nerrs++;
	}
	free(yy.dims);
