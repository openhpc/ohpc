 ...
 */
int test_ncdimdef(path) const char *path; /* name of writable netcdf to open */
{
	int nerrs = 0;
	...
	if (dimid != test.ndims - 1) {
		error("%s: ncdimdef returned %d for dim id, expected %d", pname,
		      dimid, test.ndims - 1);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	if (ncdimdef(cdfid, mm.name, mm.size) != -1) {
		error("%s: ncdimdef should not have allowed redefinition",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	if (ncdimdef(cdfid, nn.name, (long)-10) != -1) {
		error("%s: ncdimdef should not allow negative size dimension",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	if (xdimid == -1) {
		if (ncdimdef(cdfid, rec.name, rec.size) == -1) {
			error("%s: ncdimdef failed on NC_UNLIMITED dimension",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	if (ncdimid(cdfid, "rec") != test.xdimid) {
		error("%s: ncdimid returned bad value for record dimension",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	if (test.ndims >= 1) { /* if any dimensions have been defined */
		if (ncdiminq(cdfid, 0, dim.name, &dim.size) != -1) {
			error("%s: ncdiminq failed to report bad netcdf handle ",
			      pname);
			nerrs++;
		}
	}
 ...
 *    try with bad netCDF handle, check error
 */
int test_ncdimrename(path) const
	char *path; /* name of writable netcdf file to open */
{
	int nerrs = 0;
	...
	if (strcmp(dim.name, newname) != 0) {
		error("%s: ncdimrename changed name to %s instead of %s", pname,
		      dim.name, newname);
		ncclose(cdfid);
		return ++nerrs;
	}
	test.dims[pp_dim].name = (char *)erealloc(
		(void *)test.dims[pp_dim].name, strlen(newname) + 1);
	(void)strcpy(test.dims[pp_dim].name,
		     newname); /* keep test consistent */
	/* try to rename first dimension same as second, should fail */
	if (ncdimrename(cdfid, pp_dim, qq.name) != -1) {
		error("%s: ncdimrename should have failed with used name",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	if (ncdimrename(cdfid, pp_dim, "p") == -1) {
		error("%s: ncdimrename to shorter name failed in data mode",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	test.dims[pp_dim].name = (char *)erealloc(
		(void *)test.dims[pp_dim].name, strlen("p") + 1);
	...
	test.dims[pp_dim].name = (char *)erealloc(
		(void *)test.dims[pp_dim].name, strlen(pp.name) + 1);
	(void)strcpy(test.dims[pp_dim].name,
		     pp.name); /* keep test consistent */
	if (ncclose(cdfid) == -1) {
		error("%s: ncclose failed", pname);
		return ++nerrs;
	}
	...
	if (ncdimrename(cdfid, 0, dim.name) != -1) {
		error("%s: ncdimrename failed to report bad netcdf handle ",
		      pname);
		nerrs++;
	}
	free(dim.name);
