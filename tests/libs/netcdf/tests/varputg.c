...
#define TEST_FAILS(varid)                                               \
	(ncvarputg(cdfid, varid, hc.cor, hc.edg, strides, (long *)NULL, \
		   hc.vals) != -1)
		...
		if (TEST_FAILS(iv)) {
			error("%s: ncvarputg should fail for negative corner",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (TEST_FAILS(iv)) {
			error("%s: ncvarputg should fail for negative edge",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		tmp = hc.cor[id];
		hc.cor[id] =
			test.dims[id].size; /* try big coordinate, should fail */
		if (TEST_FAILS(iv)) {
			error("%s: ncvarputg should fail for too-high coordinate",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		tmp = hc.edg[id];
		hc.edg[id] =
			test.dims[id].size + 1; /* try big edge, should fail */
		if (TEST_FAILS(iv)) {
			error("%s: ncvarputg should fail for too-high edge",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (TEST_FAILS(iv)) {
			error("%s: ncvarputg should fail for non-positive stride",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (TEST_FAILS(iv)) {
			error("%s: ncvarputg should fail in define mode",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
	...
	if (TEST_FAILS(-1)) {
		error("%s: ncvarputg should fail for bad variable handle",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
	...
	if (TEST_FAILS(0)) {
		error("%s: ncvarputg failed to report bad netcdf handle",
		      pname);
		nerrs++;
	}
	if (hc.vals != NULL)
