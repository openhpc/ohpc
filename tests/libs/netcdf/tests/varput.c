	...
	char *path; /* name of writable netcdf file to open */
{
	...
	if (iv < test.nvars) { /* iv is first varid of var with dimensions */
		...
		if (ncvarput(cdfid, iv, hc.cor, hc.edg, hc.vals) != -1) {
			error("%s: ncvarput should fail for negative corner",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (ncvarput(cdfid, iv, hc.cor, hc.edg, hc.vals) != -1) {
			error("%s: ncvarput should fail for negative edge",
			      pname);
			ncclose(cdfid);
			return ++nerrs;
		}
		hc.edg[id] = tmp;
		{
			...
			hc.cor[mqv] =
				test.dims[dim]
					.size; /* try big coordinate, should fail */
			if (ncvarput(cdfid, iv, hc.cor, hc.edg, hc.vals) !=
			    -1) {
				error("%s: ncvarput should fail for too-high coordinate",
				      pname);
				ncclose(cdfid);
				return ++nerrs;
			}
			...
			tmp = hc.edg[mqv];
			hc.edg[mqv] = test.dims[dim].size +
				      1; /* try big edge, should fail */
			if (ncvarput(cdfid, iv, hc.cor, hc.edg, hc.vals) !=
			    -1) {
				error("%s: ncvarput should fail for too-high edge",
				      pname);
				ncclose(cdfid);
				return ++nerrs;
			}
	...
	if (ncvarput(cdfid, -1, hc.cor, hc.edg, hc.vals) != -1) {
		error("%s: ncvarput should fail for bad variable handle",
		      pname);
		ncclose(cdfid);
		return ++nerrs;
	}
