	...
	char *path; /* name of writable netcdf file to open */
{
	...
	for (id = 0; id < nd; id++) {
		if ((di_id[id] = ncdimdef(cdfid, di[id].name, di[id].size)) ==
		    -1) {
			error("%s: ncdimdef failed for %s, size %d", pname,
			      di[id].name, di[id].size);
			ncclose(cdfid);
			return ++nerrs;
		}
	...
	for (iv = 0; iv < nv; iv++) {
		...
		if ((va_id[iv] = ncvardef(cdfid, va[iv].name, va[iv].type,
			...
			errvar(&test,
			       &va[iv]); /* prints details about variable */
			ncclose(cdfid);
			return ++nerrs;
		}
		...
		if (ncvarinq(cdfid, va_id[iv], tmp.name, &tmp.type, &tmp.ndims,
			     tmp.dims, &tmp.natts) == -1) {
		...
		if (strcmp(tmp.name, va[iv].name) != 0 ||
		    tmp.type != va[iv].type || tmp.ndims != va[iv].ndims ||
	...
	if (ncvardef(cdfid, "badtype", BAD_TYPE, va[0].ndims, va[0].dims) !=
	    -1) {
	...
	for (iv = 0; iv < nv; iv++) {
		...
		case NC_BYTE: {
			...
			if (ncvarget1(cdfid, va_id[iv], where, (void *)&val) !=
			    -1) {
				if (val != fillval) {
					error("%s: unwritten byte not FILL_BYTE",
					      pname);
					nerrs++;
				}
			} else {
		...
		case NC_CHAR: {
			...
			if (ncvarget1(cdfid, va_id[iv], where, (void *)&val) !=
			    -1) {
				if (val != fillval) {
					error("%s: unwritten char not FILL_CHAR",
					      pname);
					nerrs++;
				}
			} else {
		...
		case NC_SHORT: {
			...
			if (ncvarget1(cdfid, va_id[iv], where, (void *)&val) !=
			    -1) {
				if (val != fillval) {
					error("%s: unwritten short not FILL_SHORT",
					      pname);
					nerrs++;
				}
			} else {
		...
		case NC_LONG: {
			...
			if (ncvarget1(cdfid, va_id[iv], where, (void *)&val) !=
			    -1) {
				if (val != fillval) {
					error("%s: unwritten long not FILL_LONG",
					      pname);
					nerrs++;
				}
			} else {
...
#define absval(x) ((x) < 0 ? -(x) : (x))
		case NC_FLOAT: {
			...
			if (ncvarget1(cdfid, va_id[iv], where, (void *)&val) !=
			    -1) {
				if (absval(val - fillval) >
				    absval(float_eps * fillval)) {
					error("%s: unwritten float not FILL_FLOAT",
					      pname);
					nerrs++;
				}
			} else {
		...
		case NC_DOUBLE: {
			...
			if (ncvarget1(cdfid, va_id[iv], where, (void *)&val) !=
			    -1) {
#define DBL_FUDGE \
	8.0 /* can be 1.0 on every platform we've seen
				   except for VAX/Ultrix 4.3 with cc */
				if (absval(val - fillval) >
				    DBL_FUDGE * absval(double_eps * fillval)) {
					error("%s: unwritten double not FILL_DOUBLE",
					      pname);
					nerrs++;
				}
			} else {
				error("%s: ncvarget1 failure for double",
				      pname);
				nerrs++;
			}
		} break;
