...
struct cdfdim *idim;
{
	...
	if (test->ndims >= MAX_NC_DIMS) {
		(void)fprintf(stderr, "%s: too many dimensions (%d)", pname,
			      test->ndims);
		return;
	}
...
struct cdfvar *ivar;
{
	...
	if (test->nvars >= MAX_NC_VARS) {
		(void)fprintf(stderr, "%s: too many variables (%d)", pname,
			      test->nvars);
		return;
	}
	...
	test->vars[test->nvars].dims =
		(int *)emalloc(sizeof(int) * ivar->ndims);
...
struct cdfatt *iatt;
{
	...
	if (test->natts >= MAX_TEST_ATTS) {
		(void)fprintf(stderr, "%s: too many attributes (%d)", pname,
			      test->natts);
		return;
	}
