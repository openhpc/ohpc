 ...
 */
int test_nccreate(path) const char *path; /* name of netCDF file to create */
{
	int nerrs = 0;
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	static struct cdfatt title = /* attribute */
		{ NC_GLOBAL, "title", NC_CHAR, LEN_OF(title_val),
		  (void *)title_val };
	FILE *temp;
	...
	if (ncopen(TEMP_FILE_NAME, NC_NOWRITE) != -1) {
		error("%s: ncopen should fail opening non-netCDF file", pname);
		return ++nerrs;
	}
	if (ncerr != NC_ENOTNC) {
	...
	/* in define mode */
	if (ncattput(ncid0, NC_GLOBAL, "title", NC_CHAR, title.len,
		     title.val) == -1) {
	...
	if ((ncid1 = ncopen(path, NC_NOWRITE)) == -1) {
		...
		if (ncredef(ncid1) != -1) {
			error("%s: cdredef should fail after NC_NOWRITE open",
			      pname);
			ncclose(ncid1);
			return ++nerrs;
		}
		...
		if (ncclose(ncid1) == -1) {
			error("%s: ncclose failed to close after second open",
			      pname);
			nerrs++;
		}
	}
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	aa.dims[0] = ii_dim;
	if ((aa_id = ncvardef(ncid, aa.name, aa.type, aa.ndims, aa.dims)) ==
	    -1) {
	...
	aa_units.var = aa_id;
	if (ncattput(ncid, aa_units.var, aa_units.name, aa_units.type,
		     aa_units.len, (void *)aa_units.val) == -1) {
		error("%s: ncattput failed", pname);
		ncclose(ncid);
		return ++nerrs;
	}
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	static struct cdfatt bb_range = /* attribute */
		{ ___, "valid_range", NC_FLOAT, LEN_OF(bb_rangev),
		  (void *)bb_rangev };
	...
	bb.dims[0] = kk_dim;
	bb.dims[1] = jj_dim;
	if ((bb_id = ncvardef(ncid, bb.name, bb.type, bb.ndims, bb.dims)) ==
	    -1) {
		error("%s: ncvardef failed", pname);
		ncclose(ncid);
		return ++nerrs;
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	if (ndims != test.ndims) {
		error("%s: ndims returned as %d, expected %d", pname, ndims,
		      test.ndims);
		nerrs++;
	}
	if (nvars != test.nvars) {
		error("%s: nvars returned as %d, expected %d", pname, nvars,
		      test.nvars);
		nerrs++;
	}
	if (ngatts != test.ngatts) {
		error("%s: ngatts returned as %d, expected %d", pname, ngatts,
		      test.ngatts);
		nerrs++;
	}
	if (xdimid != test.xdimid) {
		error("%s: xdimid returned as %d, expected %d", pname, xdimid,
		      test.xdimid);
		nerrs++;
	}
	...
	/* add dimensions */
	for (id = 0; id < nd; id++) {
		if ((dimids[id] = ncdimdef(ncid, dims[id].name,
					   dims[id].size)) == -1) {
	...
	if (ncinquire(ncid, &ndims, &nvars, &ngatts, &xdimid) != -1) {
		error("%s: ncinquire failed to report bad netcdf handle",
		      pname);
		nerrs++;
	}
	if (nerrs > 0)
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	static struct cdfatt dd_fill_val = /* attribute */
		{ ___, "fill_value", NC_SHORT, LEN_OF(dd_fill_valv),
		  (void *)dd_fill_valv };
	...
	dd.dims[0] = ll_dim;
	if ((dd_id = ncvardef(ncid0, dd.name, dd.type, dd.ndims, dd.dims)) ==
	    -1) {
		error("%s: ncvardef failed", pname);
		ncclose(ncid0);
		return ++nerrs;
	...
	/* put some data into a variable */
	{
		...
		if (ncvarput(ncid0, dd_id, dd_start, dd_edges,
			     (void *)dd_vals) == -1) {
			error("%s: ncvarput failed", pname);
			ncclose(ncid0);
			return ++nerrs;
		}
		add_data(&test, dd_id, dd_start,
			 dd_edges); /* keep test in sync */
		if (ncsync(ncid0) == -1) {
		...
		if ((ncid1 = ncopen(path, NC_NOWRITE)) == -1) {
...
#else
			fprintf(stderr,
				"Doesn't support shared access on vms\n");
#endif
		} else {
			if (ncid0 == ncid1) {
			...
			if (ncvarget(ncid1, dd_id, dd_start, dd_edges,
				     (void *)got_vals) == -1) {
				error("%s: ncvarget failed", pname);
				nerrs++;
			}
			if (dd_vals[0] != got_vals[0] ||
			    dd_vals[1] != got_vals[1]) {
	...
	char *path; /* name of writable netcdf file to open */
{
	...
	if (ncattput(ncid, NC_GLOBAL, att.name, att.type, att.len, att.val) ==
	    -1) {
		error("%s: ncattput failed", pname);
		ncclose(ncid);
		return ++nerrs;
	...
	if (ncattinq(ncid, NC_GLOBAL, att.name, &att.type, &att.len) != -1) {
		error("%s: ncabort should have restored state before ncredef",
		      pname);
		ncclose(ncid);
		return ++nerrs;
	}
