		    ...
		    const char *kind, int root = -1)
{
	if (root == -1) {
		...
		if (comm.rank() == root) {
			value = bc_value;
			std::cout << "Broadcasting " << kind << " from root "
				  << root << "...";
			std::cout.flush();
		}
...
void test_skeleton_and_content(const communicator &comm, int root = 0)
{
	...
	if (comm.rank() == root) {
		...
		std::cout << "Broadcasting integer list skeleton from root "
			  << root << "...";
		broadcast(comm, oa, root);
		std::cout << "OK." << std::endl;

		// Broadcast the content
		std::cout << "Broadcasting integer list content from root "
			  << root << "...";
		{
...
BOOST_AUTO_TEST_CASE(test_main)
{
	...
	if (comm.size() == 1) {
		std::cerr
			<< "ERROR: Must run the broadcast test with more than one "
			<< "process." << std::endl;
		MPI_Abort(comm, -1);
	}
