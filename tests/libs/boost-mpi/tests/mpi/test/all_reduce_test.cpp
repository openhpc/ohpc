...
#define BOOST_TEST_MODULE AllReduce
			   ...
			   typename Generator::result_type init, bool in_place)
{
	...
	if (comm.rank() == 0) {
		...
		std::cout << "Reducing (" << place << ") array to " << op_kind
			  << " of " << type_kind << "...";
		std::cout.flush();
	}
	...
	bool got_expected_result =
		(std::equal_range(result.begin(), result.end(),
				  expected_result) ==
...
struct wrapped_int {
	...
	explicit wrapped_int(int value)
		: value(value)
	{
	}
...
struct wrapped_int_generator {
	...
	wrapped_int_generator(int base = 1)
		: base(base)
	{
	}

	wrapped_int operator()(int p) const
	{
		return wrapped_int(base + p);
	}
...
BOOST_AUTO_TEST_CASE(test_main)
{
	...
	all_reduce_test(comm, int_generator(), "integers", std::plus<int>(),
			"sum", 0);
	all_reduce_test(comm, int_generator(), "integers",
			std::multiplies<int>(), "product", 1);
	...
	all_reduce_test(comm, int_generator(17), "integers",
			secret_int_bit_and(), "bitwise and", -1);
