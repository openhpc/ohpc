		       ...
		       int num_values, const char *kind, bool composite)
{
	...
	if (!composite) {
		...
		nonblocking_test(comm, values, num_values, kind,
				 mk_test_all_keep);
	}
		      ...
		      const char *kind, method_kind method)
{
	...
	if (comm.rank() == 0) {
		std::cout << "Testing " << method_kind_names[method] << " with "
			  << kind << "...";
		std::cout.flush();
	}
	...
	reqs.push_back(
		comm.isend((comm.rank() + 1) % comm.size(), 0, values[0]));
	...
	if (method != mk_wait_any && method != mk_test_any) {
		...
		reqs.push_back(comm.irecv(
			(comm.rank() + comm.size() - 1) % comm.size(), 1));
#endif

		// Send/receive an array
		reqs.push_back(comm.isend((comm.rank() + 1) % comm.size(), 2,
					  values, num_values));
		reqs.push_back(comm.irecv(
			(comm.rank() + comm.size() - 1) % comm.size(), 2,
	...
	case mk_test_all_keep: {
		std::vector<status> stats;
		while (!test_all(reqs.begin(), reqs.end(),
	...
	if (method != mk_wait_any && method != mk_test_any)
		BOOST_CHECK(std::equal(incoming_values.begin(),
				       incoming_values.end(), values));
}
...
int test_main(int argc, char *argv[])
{
	...
	gps_position gps_array[2] = { gps_position(17, 42, .06),
