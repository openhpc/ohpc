	       ...
	       int root = 0)
{
	...
	if (rank == root) {
		std::cout << "Passing " << kind << " around a ring from root "
			  << root << "...";
		     ...
		     const char *kind, int root = 0)
{
	...
	if (rank == root) {
		std::cout << "Passing " << kind
			  << " array around a ring from root " << root << "...";
...
BOOST_AUTO_TEST_CASE(test_main)
{
	...
	if (comm.size() == 1) {
		std::cerr
			<< "ERROR: Must run the ring test with more than one process."
			<< std::endl;
		MPI_Abort(comm, -1);
	}
	...
	gps_position gps_position_array[2] = { gps_position(39, 16, 20.2799),
