...
int main(int argc, char *argv[])
{
	...
	if (iam >= 0 && iam < 6) { /* I am in grid 1. */
		...
		/* ------------------------------------------------------------
           ...
           ------------------------------------------------------------*/
		dcreate_matrix_postfix(&A, nrhs, &b, &ldb, &xtrue, &ldx, fp,
				       postfix, &grid1);
	   ...
	   ------------------------------------------------------------*/
		...
		pdinf_norm_error(iam, ((NRformat_loc *)A.Store)->m_loc, nrhs, b,
				 ldb, xtrue, ldx, grid1.comm);
