...
int main(int argc, char *argv[])
{
	...
	/* prototypes */
	extern int dcreate_matrix_perturbed(SuperMatrix *, int, double **,
					    int *, double **, int *, FILE *,
					    gridinfo_t *);
	extern int dcreate_matrix_perturbed_postfix(SuperMatrix *, int,
						    double **, int *, double **,
						    int *, FILE *, char *,
						    gridinfo_t *);
	...
	Destroy_CompRowLoc_Matrix_dist(
		&A); /* Deallocate storage of matrix A.  */
	...
	if (!(fp = fopen(*cpp, "r")))
		ABORT("File does not exist");
	dcreate_matrix_perturbed_postfix(&A, nrhs, &b1, &ldb, &xtrue1, &ldx, fp,
					 postfix, &grid);

	PStatInit(&stat); /* Initialize the statistics variables. */

	/* Solve the linear system. */
	pdgssvx(&options, &A, &ScalePermstruct, b1, ldb, nrhs, &grid, &LUstruct,
		&SOLVEstruct, berr, &stat, &info);
	...
	Destroy_CompRowLoc_Matrix_dist(
		&A); /* Deallocate storage of matrix A.  */
	dDestroy_LU(n, &grid,
		    &LUstruct); /* Deallocate storage associated with    
					the L and U matrices.               */
