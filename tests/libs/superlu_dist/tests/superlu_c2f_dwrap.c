			   ...
			   int *PrintStat)
{
	...
	*ReplaceTinyPivot =
		(int)((superlu_dist_options_t *)*opt)->ReplaceTinyPivot;
	*SolveInitialized =
		(int)((superlu_dist_options_t *)*opt)->SolveInitialized;
	*RefineInitialized =
		(int)((superlu_dist_options_t *)*opt)->RefineInitialized;
			   ...
			   int *PrintStat)
{
	...
	((superlu_dist_options_t *)*opt)->IterRefine =
		(IterRefine_t)*IterRefine;
	((superlu_dist_options_t *)*opt)->Trans = (trans_t)*Trans;
	((superlu_dist_options_t *)*opt)->ReplaceTinyPivot =
		(yes_no_t)*ReplaceTinyPivot;
	((superlu_dist_options_t *)*opt)->SolveInitialized =
		(yes_no_t)*SolveInitialized;
	((superlu_dist_options_t *)*opt)->RefineInitialized =
...
void f_superlu_gridmap(int *Bcomm, int_t *nprow, int_t *npcol, int_t *usermap,
		       int_t *ldumap, fptr *grid)
{
...
void f_dCreate_CompRowLoc_Mat_dist(fptr *A, int *m, int *n, int *nnz_loc,
				   ...
				   int *dtype, int *mtype)
{
	dCreate_CompRowLoc_Matrix_dist((SuperMatrix *)*A, *m, *n, *nnz_loc,
				       *m_loc, *fst_row, (double *)nzval,
				       colind, rowptr, (Stype_t)*stype,
	       ...
	       fptr *SOLVEstruct, double *berr, fptr *stat, int *info)
{
	pdgssvx((superlu_dist_options_t *)*options, (SuperMatrix *)*A,
		...
		(dSOLVEstruct_t *)*SOLVEstruct, berr, (SuperLUStat_t *)*stat,
		info);
