//
// https://code.google.com/p/trilinos/wiki/TeuchosPL
//
...
int main(int argc, char *argv[])
{
	...
	typedef Teuchos::StringToIntegralParameterEntryValidator<int>
		validator_type;
	RCP<validator_type> solverValidator = rcp(new validator_type(
		tuple<std::string>("GMRES", "CG", "TFQMR"), "Solver"));

	// Set the "Solver" parameter.  Specify the above validator;
	// it will validate the "GMRES" input right here.
	My_List.set("Solver", "GMRES", "The type of solver to use.",
		    solverValidator);
	...
	My_List.set("Tolerance", static_cast<float>(1e-10),
	...
	My_List.set("Initial Guess", rcp_Array,
		    "The initial guess as a RCP to an Array.");
	...
	ParameterList &Prec_List =
		My_List.sublist("Preconditioner", false,
  ...
  */
	...
	std::string solver = solverValidator->validateString(
	...
	My_List.print(std::cout, Teuchos::ParameterList::PrintOptions()
					 .showDoc(true)
					 .indent(2)
					 .showTypes(true));
