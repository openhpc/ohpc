...
createMatrix(const Teuchos::RCP<const typename CrsMatrixType::map_type> &map)
{
	...
	for (iter_type it = myGlobalElements.begin();
	     it != myGlobalElements.end(); ++it) {
		const LO i_local = *it;
	     ...
	     std::ostream &out)
{
	...
	{
		const size_t numLocalIndices =
			(myRank == 0) ? numGlobalIndices : 0;
		procZeroMap = rcp(new map_type(
			numGlobalIndices, numLocalIndices, indexBase, comm));
	}
	...
	RCP<const crs_matrix_type> A =
		createMatrix<crs_matrix_type>(procZeroMap);
	//
...
int main(int argc, char *argv[])
{
	...
	Tpetra::ScopeGuard tpetraScope(&argc, &argv);
	{
		...
		exportTimer = TimeMonitor::getNewCounter(
			"Sparse matrix redistribution");
