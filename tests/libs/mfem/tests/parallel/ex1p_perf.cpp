...
template <> struct geom_t<2> {
	static const Geometry::Type value = Geometry::SQUARE;
};
template <> struct geom_t<3> {
...
template <int dim> struct ex1_t {
	...
	using HPCBilinearForm =
		TBilinearForm<mesh_t, sol_fes_t, int_rule_t, integ_t>;

	static int run(Mesh *mesh, int ser_ref_levels, int par_ref_levels,
...
int main(int argc, char *argv[])
{
	...
	args.AddOption(
		&par_ref_levels, "-rp", "--refine-parallel",
	...
	args.AddOption(
		&perf, "-perf", "--hpc-version", "-std", "--standard-version",
		"Enable high-performance, tensor-based, assembly/evaluation.");
	args.AddOption(
		&matrix_free, "-mf", "--matrix-free", "-asm", "--assembly",
	...
	if (!strcmp(pc, "ho")) {
		pc_choice = PCType::HO;
	} else if (!strcmp(pc, "lor")) {
		pc_choice = PCType::LOR;
	} else if (!strcmp(pc, "none")) {
		pc_choice = PCType::NONE;
	} else {
	...
	if (myid == 0) {
		cout << "\nMFEM SIMD width: "
		     << MFEM_SIMD_BYTES / sizeof(double) << " doubles\n"
		     << endl;
	}
	...
	if (myid == 0) {
		cout << "Using " << BasisType::Name(basis) << " basis ..."
		     << endl;
	}
	...
	if (dim == 2) {
		return ex1_t<2>::run(mesh, ser_ref_levels, par_ref_levels,
				     order, basis, static_cond, pc_choice, perf,
				     matrix_free, visualization, visport);
	} else if (dim == 3) {
		return ex1_t<3>::run(mesh, ser_ref_levels, par_ref_levels,
				     order, basis, static_cond, pc_choice, perf,
				     matrix_free, visualization, visport);
	} else {
		    ...
		    int visport)
{
		...
		if (!mesh_t::MatchesGeometry(*mesh)) {
			if (myid == 0) {
				cout << "The given mesh does not match the optimized 'geom' parameter.\n"
				     << "Recompile with suitable 'geom' value."
				     << endl;
			}
			delete mesh;
			return 4;
			...
			if (myid == 0) {
				cout << "Switching the mesh curvature to match the "
				     << "optimized value (order " << mesh_p
				     << ") ..." << endl;
			}
			mesh->SetCurvature(mesh_p, false, -1,
					   Ordering::byNODES);
		}
	}
	...
	//    option '--refine-serial'.
	{
		int ref_levels =
			(ser_ref_levels != -1) ?
				ser_ref_levels :
				(int)floor(log(10000. / mesh->GetNE()) /
					   log(2.) / dim);
	...
	if (order > 0) {
		...
		if (myid == 0) {
			cout << "Using isoparametric FEs: " << fec->Name()
			     << endl;
		}
	} else {
	...
	if (perf && !sol_fes_t::Matches(*fespace)) {
		if (myid == 0) {
			cout << "The given order does not match the optimized parameter.\n"
			     << "Recompile with suitable 'sol_p' value."
			     << endl;
		}
		delete fespace;
		delete fec;
	...
	if (myid == 0) {
		...
		cout << "Time per CG step: "
		     << tic_toc.RealTime() / pcg->GetNumIterations() << "s."
		     << endl;
	}
