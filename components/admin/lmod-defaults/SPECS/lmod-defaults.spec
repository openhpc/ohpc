...
if [ "${1}" -ge 1 ]; then
    ...
    if [ -n "${version}" ]; then
        ...
        if [ -f "${module_dir}/${moduleFile}" ]; then
            # Remove default symlink set by previous package releases
            [ -L "${module_dir}/default" ] && rm -f "${module_dir}/default"
            # Establish latest default
            cat << EOF > "${module_dir}/.version"
#%Module1.0#####################################################################
set     ModulesVersion      "${moduleFile}"
EOF
        fi
    fi
fi
...
# Only on package erase: on upgrade, the .version file has already been
# regenerated by the %%post scriptlet of the new package release
if [ "${1}" -eq 0 ]; then
    module_dir="%{mpi_module_dir}"
    # Remove default symlink set by previous package releases
    [ -L "${module_dir}/default" ] && rm -f "${module_dir}/default"
    rm -f "${module_dir}/.version"
fi
