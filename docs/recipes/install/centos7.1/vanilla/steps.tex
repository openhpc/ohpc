...
[sms](*\#*) ifconfig ${sms_eth_internal} ${master_ip} netmask ${internal_netmask} up
...
[sms](*\#*) echo "${master_ip}:/home /home nfs nfsvers=3,rsize=1024,wsize=1024,cto 0 0" >> $CHROOT/etc/fstab
[sms](*\#*) echo "${master_ip}:/opt/ohpc/pub /opt/ohpc/pub nfs nfsvers=3 0 0" >> $CHROOT/etc/fstab

# Identify resource manager hostname on master host
[sms](*\#*) perl -pi -e "s/ControlMachine=\S+/ControlMachine=${master_name}/" /etc/slurm/slurm.conf
...
[sms](*\#*) echo "server ${master_ip}" >> $CHROOT/etc/ntp.conf
\end{lstlisting}
% end_ohpc_run
...
\begin{tcolorbox}[]
\small
Slurm requires three explicit configuration parameters to define the allocation of
user threads to a particular hardware resource: Sockets, CoresPerSocket, and
ThreadsPerCore. The default configuration file assumes dual-socket, 8 cores per
socket, and two threads per core. Make sure that these values in
/etc/slurm/slurm.conf match your particular hardware.
\end{tcolorbox}
\end{center}
...
%% [FSP-469] [sms](*\#*) perl -pi -e "s/Servername = (<.+>)/Servername = ${master_ip}/" /etc/orcmdb_psql.ini
%% [FSP-469] 
...
\paragraph{Enable forwarding of system logs}
...
[sms](*\#*) wwsh -y set network --path /etc/sysconfig/network --mode=0644 --uid=0
