...
process which requires external network connectivity on your chosen {\em
 master} host. If the master host cannot reach the public CentOS mirrors, or if 
you prefer to access a locally cached mirror, the steps below include an example 
to update the OS mirror used by \texttt{wwmkchroot} to use an alternate location 
defined by a \texttt{\$\{BOS\_MIRROR\}} environment variable.
...
%       Replace the \texttt{[l]pxelinux.0} from 
