\newcolumntype{C}[1]{>{\centering}p{#1}}
\newcolumntype{L}[1]{>{\raggedleft}p{#1}}
\newcolumntype{C}[1]{>{\centering}p{#1}}
\newcolumntype{L}[1]{>{\raggedleft}p{#1}}
\small
\begin{tabularx}{\textwidth}{L{\firstColWidth{}}|C{\secondColWidth{}}|X}
\toprule
{\bf RPM Package Name} & {\bf Version} & {\bf Info/URL}  \\
\midrule

% <-- begin entry for boost
boost-arm1-mpich-ohpc &
\multirow{6}{*}{1.81.0} &
\multirow{10}{\linewidth}{Free peer-reviewed portable C++ source libraries. \newline {\color{logoblue} \url{http://www.boost.org}}}\\
boost-arm1-openmpi4-ohpc &
& \\
boost-gnu12-mpich-ohpc &
& \\
boost-gnu12-openmpi4-ohpc &
& \\
boost-gnu13-mpich-ohpc &
& \\
boost-gnu13-openmpi5-ohpc &
& \\
\cline{1-2} boost-gnu14-mpich-ohpc &\multirow{4}{*}{1.88.0}
& \\
boost-gnu14-openmpi5-ohpc &
& \\
boost-gnu15-mpich-ohpc &
& \\
boost-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for boost

% <-- begin entry for fftw
fftw-arm1-mpich-ohpc &
\multirow{10}{*}{3.3.10} &
\multirow{10}{\linewidth}{A Fast Fourier Transform library. \newline {\color{logoblue} \url{http://www.fftw.org}}}\\
fftw-arm1-openmpi4-ohpc &
& \\
fftw-gnu12-mpich-ohpc &
& \\
fftw-gnu12-openmpi4-ohpc &
& \\
fftw-gnu13-mpich-ohpc &
& \\
fftw-gnu13-openmpi5-ohpc &
& \\
fftw-gnu14-mpich-ohpc &
& \\
fftw-gnu14-openmpi5-ohpc &
& \\
fftw-gnu15-mpich-ohpc &
& \\
fftw-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for fftw

% <-- begin entry for hypre
hypre-arm1-mpich-ohpc &
\multirow{6}{*}{2.18.1} &
\multirow{10}{\linewidth}{Scalable algorithms for solving linear systems of equations. \newline {\color{logoblue} \url{http://www.llnl.gov/casc/hypre}}}\\
hypre-arm1-openmpi4-ohpc &
& \\
hypre-gnu12-mpich-ohpc &
& \\
hypre-gnu12-openmpi4-ohpc &
& \\
hypre-gnu13-mpich-ohpc &
& \\
hypre-gnu13-openmpi5-ohpc &
& \\
\cline{1-2} hypre-gnu14-mpich-ohpc &\multirow{4}{*}{2.33.0}
& \\
hypre-gnu14-openmpi5-ohpc &
& \\
hypre-gnu15-mpich-ohpc &
& \\
hypre-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for hypre

% <-- begin entry for mfem
mfem-arm1-mpich-ohpc &
\multirow{10}{*}{4.4} &
\multirow{10}{\linewidth}{Lightweight, general, scalable C++ library for finite element methods. \newline {\color{logoblue} \url{http://mfem.org}}}\\
mfem-arm1-openmpi4-ohpc &
& \\
mfem-gnu12-mpich-ohpc &
& \\
mfem-gnu12-openmpi4-ohpc &
& \\
mfem-gnu13-mpich-ohpc &
& \\
mfem-gnu13-openmpi5-ohpc &
& \\
mfem-gnu14-mpich-ohpc &
& \\
mfem-gnu14-openmpi5-ohpc &
& \\
mfem-gnu15-mpich-ohpc &
& \\
mfem-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for mfem

% <-- begin entry for mumps
mumps-arm1-mpich-ohpc &
\multirow{8}{*}{5.2.1} &
\multirow{10}{\linewidth}{A MUltifrontal Massively Parallel Sparse direct Solver. \newline {\color{logoblue} \url{http://graal.ens-lyon.fr/MUMPS}}}\\
mumps-arm1-openmpi4-ohpc &
& \\
mumps-gnu12-mpich-ohpc &
& \\
mumps-gnu12-openmpi4-ohpc &
& \\
mumps-gnu13-mpich-ohpc &
& \\
mumps-gnu13-openmpi5-ohpc &
& \\
mumps-gnu14-mpich-ohpc &
& \\
mumps-gnu14-openmpi5-ohpc &
& \\
\cline{1-2} mumps-gnu15-mpich-ohpc &\multirow{2}{*}{5.8.1}
& \\
mumps-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for mumps

% <-- begin entry for opencoarrays
opencoarrays-gnu12-mpich-ohpc &
\multirow{4}{*}{2.10.0} &
\multirow{6}{\linewidth}{ABI to leverage the parallel programming features of the Fortran 2018 DIS. \newline {\color{logoblue} \url{http://www.opencoarrays.org}}}\\
opencoarrays-gnu12-openmpi4-ohpc &
& \\
opencoarrays-gnu13-mpich-ohpc &
& \\
opencoarrays-gnu13-openmpi5-ohpc &
& \\
\cline{1-2} opencoarrays-gnu14-mpich-ohpc &\multirow{2}{*}{2.10.2}
& \\
opencoarrays-gnu14-openmpi5-ohpc &
& \\
\hline
% <-- end entry for opencoarrays

% <-- begin entry for petsc
petsc-arm1-mpich-ohpc &
\multirow{8}{*}{3.18.1} &
\multirow{10}{\linewidth}{Portable Extensible Toolkit for Scientific Computation. \newline {\color{logoblue} \url{http://www.mcs.anl.gov/petsc}}}\\
petsc-arm1-openmpi4-ohpc &
& \\
petsc-gnu12-mpich-ohpc &
& \\
petsc-gnu12-openmpi4-ohpc &
& \\
petsc-gnu13-mpich-ohpc &
& \\
petsc-gnu13-openmpi5-ohpc &
& \\
petsc-gnu14-mpich-ohpc &
& \\
petsc-gnu14-openmpi5-ohpc &
& \\
\cline{1-2} petsc-gnu15-mpich-ohpc &\multirow{2}{*}{3.23.5}
& \\
petsc-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for petsc

% <-- begin entry for ptscotch
ptscotch-arm1-mpich-ohpc &
\multirow{6}{*}{7.0.1} &
\multirow{10}{\linewidth}{Graph, mesh and hypergraph partitioning library using MPI. \newline {\color{logoblue} \url{http://www.labri.fr/perso/pelegrin/scotch}}}\\
ptscotch-arm1-openmpi4-ohpc &
& \\
ptscotch-gnu12-mpich-ohpc &
& \\
ptscotch-gnu12-openmpi4-ohpc &
& \\
ptscotch-gnu13-mpich-ohpc &
& \\
ptscotch-gnu13-openmpi5-ohpc &
& \\
\cline{1-2} ptscotch-gnu14-mpich-ohpc &\multirow{4}{*}{7.0.7}
& \\
ptscotch-gnu14-openmpi5-ohpc &
& \\
ptscotch-gnu15-mpich-ohpc &
& \\
ptscotch-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for ptscotch

% <-- begin entry for scalapack
scalapack-arm1-mpich-ohpc &
\multirow{6}{*}{2.2.0} &
\multirow{10}{\linewidth}{A subset of LAPACK routines redesigned for heterogeneous computing. \newline {\color{logoblue} \url{https://netlib.org/scalapack}}}\\
scalapack-arm1-openmpi4-ohpc &
& \\
scalapack-gnu12-mpich-ohpc &
& \\
scalapack-gnu12-openmpi4-ohpc &
& \\
scalapack-gnu13-mpich-ohpc &
& \\
scalapack-gnu13-openmpi5-ohpc &
& \\
\cline{1-2} scalapack-gnu14-mpich-ohpc &\multirow{4}{*}{2.2.2}
& \\
scalapack-gnu14-openmpi5-ohpc &
& \\
scalapack-gnu15-mpich-ohpc &
& \\
scalapack-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for scalapack

% <-- begin entry for slepc
slepc-arm1-mpich-ohpc &
\multirow{8}{*}{3.18.0} &
\multirow{10}{\linewidth}{A library for solving large scale sparse eigenvalue problems. \newline {\color{logoblue} \url{http://slepc.upv.es}}}\\
slepc-arm1-openmpi4-ohpc &
& \\
slepc-gnu12-mpich-ohpc &
& \\
slepc-gnu12-openmpi4-ohpc &
& \\
slepc-gnu13-mpich-ohpc &
& \\
slepc-gnu13-openmpi5-ohpc &
& \\
slepc-gnu14-mpich-ohpc &
& \\
slepc-gnu14-openmpi5-ohpc &
& \\
\cline{1-2} slepc-gnu15-mpich-ohpc &\multirow{2}{*}{3.23.0}
& \\
slepc-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for slepc

% <-- begin entry for superlu_dist
superlu\_dist-arm1-mpich-ohpc &
\multirow{10}{*}{6.4.0} &
\multirow{10}{\linewidth}{A general purpose library for the direct solution of linear equations. \newline {\color{logoblue} \url{https://portal.nersc.gov/project/sparse/superlu}}}\\
superlu\_dist-arm1-openmpi4-ohpc &
& \\
superlu\_dist-gnu12-mpich-ohpc &
& \\
superlu\_dist-gnu12-openmpi4-ohpc &
& \\
superlu\_dist-gnu13-mpich-ohpc &
& \\
superlu\_dist-gnu13-openmpi5-ohpc &
& \\
superlu\_dist-gnu14-mpich-ohpc &
& \\
superlu\_dist-gnu14-openmpi5-ohpc &
& \\
superlu\_dist-gnu15-mpich-ohpc &
& \\
superlu\_dist-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for superlu_dist

% <-- begin entry for trilinos
trilinos-gnu12-mpich-ohpc &
\multirow{6}{*}{13.4.0} &
\multirow{8}{\linewidth}{A collection of libraries of numerical algorithms. \newline {\color{logoblue} \url{https://trilinos.org}}}\\
trilinos-gnu12-openmpi4-ohpc &
& \\
trilinos-gnu13-mpich-ohpc &
& \\
trilinos-gnu13-openmpi5-ohpc &
& \\
trilinos-gnu14-mpich-ohpc &
& \\
trilinos-gnu14-openmpi5-ohpc &
& \\
\cline{1-2} trilinos-gnu15-mpich-ohpc &\multirow{2}{*}{16.1.0}
& \\
trilinos-gnu15-openmpi5-ohpc &
& \\
\hline
% <-- end entry for trilinos

\bottomrule
\end{tabularx}
