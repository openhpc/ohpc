\newcolumntype{C}[1]{>{\centering}p{#1}}
\newcolumntype{L}[1]{>{\raggedleft}p{#1}}
\small
\begin{tabularx}{\textwidth}{L{\firstColWidth{}}|C{\secondColWidth{}}|X}
\toprule
{\bf RPM Package Name} & {\bf Version} & {\bf Info/URL}  \\
\midrule

pdtoolkit-gnu14-ohpc &
& \\
pdtoolkit-intel-ohpc &
& \\
\hline
% <-- end entry for pdtoolkit

% <-- begin entry for scalasca
scalasca-gnu12-impi-ohpc &
\multirow{9}{*}{2.5} &
\multirow{20}{\linewidth}{Toolset for performance analysis of large-scale parallel applications. \newline {\color{logoblue} \url{http://www.scalasca.org}}}\\
scalasca-gnu12-mpich-ohpc &
& \\
scalasca-gnu12-mvapich2-ohpc &
& \\
scalasca-gnu12-openmpi4-ohpc &
& \\
scalasca-gnu13-impi-ohpc &
& \\
scalasca-gnu13-mpich-ohpc &
& \\
scalasca-gnu13-mvapich2-ohpc &
& \\
scalasca-gnu13-openmpi5-ohpc &
& \\
scalasca-intel-openmpi4-ohpc &
& \\
\cline{1-2} scalasca-gnu14-impi-ohpc &\multirow{11}{*}{2.6.2}
& \\
scalasca-gnu14-mpich-ohpc &
& \\
scalasca-gnu14-mvapich2-ohpc &
& \\
scalasca-gnu14-openmpi5-ohpc &
& \\
scalasca-gnu15-mpich-ohpc &
& \\
scalasca-gnu15-mvapich2-ohpc &
& \\
scalasca-gnu15-openmpi5-ohpc &
& \\
scalasca-intel-impi-ohpc &
& \\
scalasca-intel-mpich-ohpc &
& \\
scalasca-intel-mvapich2-ohpc &
& \\
scalasca-intel-openmpi5-ohpc &
& \\
\hline
% <-- end entry for scalasca

% <-- begin entry for scorep
scorep-gnu12-impi-ohpc &
\multirow{9}{*}{7.1} &
\multirow{20}{\linewidth}{Scalable Performance Measurement Infrastructure for Parallel Codes. \newline {\color{logoblue} \url{http://www.vi-hps.org/projects/score-p}}}\\
scorep-gnu12-mpich-ohpc &
& \\
scorep-gnu12-mvapich2-ohpc &
& \\
scorep-gnu12-openmpi4-ohpc &
& \\
scorep-gnu13-impi-ohpc &
& \\
scorep-gnu13-mpich-ohpc &
& \\
scorep-gnu13-mvapich2-ohpc &
& \\
scorep-gnu13-openmpi5-ohpc &
& \\
scorep-intel-openmpi4-ohpc &
& \\
\cline{1-2} scorep-gnu14-impi-ohpc &\multirow{11}{*}{9.0}
& \\
scorep-gnu14-mpich-ohpc &
& \\
scorep-gnu14-mvapich2-ohpc &
& \\
scorep-gnu14-openmpi5-ohpc &
& \\
scorep-gnu15-mpich-ohpc &
& \\
scorep-gnu15-mvapich2-ohpc &
& \\
scorep-gnu15-openmpi5-ohpc &
& \\
scorep-intel-impi-ohpc &
& \\
scorep-intel-mpich-ohpc &
& \\
scorep-intel-mvapich2-ohpc &
& \\
scorep-intel-openmpi5-ohpc &
& \\
\hline
% <-- end entry for scorep

% <-- begin entry for tau
tau-gnu12-impi-ohpc &
\multirow{17}{*}{2.31.1} &
\multirow{17}{\linewidth}{Tuning and Analysis Utilities Profiling Package. \newline {\color{logoblue} \url{http://www.cs.uoregon.edu/research/tau/home.php}}}\\
tau-gnu12-mpich-ohpc &
& \\
tau-gnu12-mvapich2-ohpc &
& \\
tau-gnu12-openmpi4-ohpc &
& \\
tau-gnu13-impi-ohpc &
& \\
tau-gnu13-mpich-ohpc &
& \\
tau-gnu13-mvapich2-ohpc &
& \\
tau-gnu13-openmpi5-ohpc &
& \\
tau-gnu14-impi-ohpc &
& \\
tau-gnu14-mpich-ohpc &
& \\
tau-gnu14-mvapich2-ohpc &
& \\
tau-gnu14-openmpi5-ohpc &
& \\
tau-intel-impi-ohpc &
& \\
tau-intel-mpich-ohpc &
& \\
tau-intel-mvapich2-ohpc &
& \\
tau-intel-openmpi4-ohpc &
& \\
tau-intel-openmpi5-ohpc &
& \\
\hline
% <-- end entry for tau

\bottomrule
\end{tabularx}
