...
\noindent {\large {\em Config Management Edition using LosF}}
...
%\newpage
\section{Install FSP Provisioning Components}
...
[master]$ export CHROOT=/opt/fsp/admin/images/sles11sp3/base
...
network settings for four example compute nodes. First, the desired IP settings
for each compute node are added to the LosF networking configuration stored in
the ips.cluster file. Then, each host is registered for addition via ``losf
add'' which performs the necessary configuration steps required for the
underlying provisioning system (Warewulf in this example).  Again, note the use
of variables for the desired compute node IP and MAC addresses which should be
modified for local settings.
...
\subsubsection{Initialize BOS image}
\subsubsection{Add FSP components}
\subsubsection{Customize settings}
\subsubsection{Assemble vNFS image}

\subsection{Boot compute nodes}

\input{../../common/reset_computes} 
An example to reboot four compute hosts using LosF provided ipmi utilities is
shown below.  Note that the \texttt{ireset} command requires that the
...
verify that the compute hosts are available via ssh, or via LosF's parallel
cluster fork tool (\texttt{koomie\_cf}) to spawn commands across multiple hosts
in parallel.
%access multiple hosts ssh tools to multiple
%hosts. 
For example, to wait 100 seconds, and run the \texttt{uptime} command on the newly imaged
compute hosts, execute the following:
