...
\lhead{{\em 2015 \FSP{} Install Guide}}
...
[master]$ wget -P /etc/yum.repos.d $FSP_mirror/ForestPeak:/15.16:/Factory/CentOS-7_Intel/ForestPeak:15.16:Factory.repo
\end{lstlisting}
% end_fsp_run

% begin_fsp_run
% fsp_validation_comment - disable proxy for internal repo (CI only command)
% [master]$ echo "proxy=_none_" >> /etc/yum.repos.d/ForestPeak:15.16:Factory.repo
% 
% [master]$ yum repolist
% [master]$ perl -i -pe 's/type="group"/type="group_gz"/' /var/cache/yum/x86_64/7/ForestPeak_15.16_Factory/repomd.xml
% end_fsp_run  
...
\begin{lstlisting}[language=bash,keywords={}]
[master]$ yum -y install libibverbs-utils libmlx4 librdmacm rdma
# yum -y install libibmad5 librdmacm1 rdma
...
default location for this in \FSP{} is in \texttt{/opt/fsp/admin/images/centos7}.
...
% begin_fsp_run
% fsp_validation_comment - fix grouplist for chroot
% [master]$ perl -i -pe 's/type="group"/type="group_gz"/' $CHROOT/var/cache/yum/x86_64/7/ForestPeak_15.16_Factory/repomd.xml
% end_fsp_run
...
# Add IB support
[master]$ yum -y --installroot=$CHROOT install libibverbs-utils libmlx4 librdmacm rdma
# yum -y --installroot=$CHROOT install libibmad5 librdmacm1 rdma
...
\begin{lstlisting}[language=bash,keywords={},upquote=true]
[master]$ yum -y --installroot=$CHROOT install -t pattern fsp-orcm-client
...
% [master]$    wwsh -y provision set c* --vnfs=centos-7 --bootstrap=`uname -r` --files=dynamic_hosts,passwd,group,shadow,munge.key 
