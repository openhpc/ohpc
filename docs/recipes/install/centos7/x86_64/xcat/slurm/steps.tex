...
\newcommand{\addrepo}{yum-config-manager --add-repo}
...
distros. To enable for local use, issue the following:

% begin_ohpc_run
\begin{lstlisting}[language=bash,keywords={},basicstyle=\fontencoding{T1}\fontsize{8.0}{10}\ttfamily,literate={ARCH}{\arch{}}1 {-}{-}1]
[sms](*\#*) wget -P /etc/yum.repos.d https://xcat.org/files/xcat/repos/yum/xcat-dep/rh7/ARCH/xCAT-dep.repo
\end{lstlisting}
% end_ohpc_run
...
\subsubsection{Idenitify sync files} \label{sec:file_import}
...
\input{common/add_xcat_hosts_slurm}
