...
\vspace*{2cm}
\noindent {\LARGE \color{RoyalBlue} \fontfamily{phv}\selectfont 2015 Forest Peak (v15.05)} \vspace*{0.1cm} \\
...
[master]$ zypper -n install lustre-client-fsp lustre-fsp-modules
...
[master]$ zypper --root=$CHROOT -n install lustre-client-fsp lustre-fsp-modules
...
FFTW and HDF5 (including serial and parallel I/O support), and the GNU
Scientific Library (GSL). Again, multiple builds of each package are available
in the FSP repository to support multiple compiler and MPI family combinations
where appropriate. The general naming convention for builds provided by FSP is
to append the compiler and MPI family name that the library was built against
directly into the package name. For example, libraries that do not require MPI
as part of the build process adopt the following RPM name: \\
...
immediately available for use on the cluster after installation on the {\em
  master} host.  For convenience, FSP provides package aliases for the FFTW,
GSL, and HDF5 libraries that can be used to install all of the available
compiler/MPI family permutations. To install all of the available package
builds for these libraries, issue the following:
...
[master]$ zypper -n install -t pattern fsp-gsl      # installs 2 (serial) gsl packages
