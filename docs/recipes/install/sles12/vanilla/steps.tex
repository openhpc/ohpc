...
[master]$ perl -pi -e "s/Servername = (\S+)/Servername = <master_ip>/" /etc/orcmdb_psql.ini
...
[master]$ cp /etc/postgresql.orcm.conf /var/lib/pgsql/data
[master]$ cp /etc/pg_hba.orcm.conf /var/lib/pgsql/data
...
# Create ORCM db (note switch to postgres user)
[master]$ su - postgres
[postgres]$ psql --command "CREATE USER orcmuser WITH SUPERUSER PASSWORD 'orcmpassword';"
[postgres]$ createdb -O orcmuser orcmdb
[postgres]$ psql --username=orcmuser --dbname=orcmdb -f /etc/orcmdb_psql.sql
[postgres]$ exit
