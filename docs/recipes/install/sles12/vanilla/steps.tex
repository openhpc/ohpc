...
With the InifiniBand drivers included, you can also enable IPoIB funcionality
...
[master]$ echo "drivers += nfs_acl, nfsv3, nfsv4, coretemp" >> $WW_CONF
...
Additionally, note that your default SLES environment may not allow loading of
...
[master]$ mount -t lustre -o localflock <mgs_node>:/<lustre_fs_name> /mnt/lustre
\end{lstlisting}
% end_fsp_run
...
[master]$ echo "<mgs_node>:/<lustre_fs_name> /mnt/lustre lustre defaults,_netdev,noauto,localflock 0 0" \
     >> $CHROOT/etc/fstab
\end{lstlisting}
% end_fsp_run
...
% begin_fsp_run
\begin{lstlisting}[language=bash,literate={-}{-}1,keywords={},upquote=true]
[master]$ wwsh file import /opt/fsp/pub/examples/network/sles/ifcfg-ib0
...
[master]$ wwsh provision set c[1-4] --fileadd=ifcfg-ib0.ww
