...
\begin{lstlisting}[language=bash]
[master]$ zypper -n install -t pattern fsp-hdf5     # installs 2 HDF5 packages
[master]$ zypper -n install -t pattern fsp-gsl      # installs 2 GSL packages
[master]$ zypper -n install -t pattern fsp-metis    # installs 2 METIS packages
[master]$ zypper -n install -t pattern fsp-fftw     # installs 6 FFTW packages
[master]$ zypper -n install -t pattern fsp-phdf5    # installs 6 (parallel) HDF5 packages
[master]$ zypper -n install -t pattern fsp-phdf5    # installs 6 PETSC packages
[master]$ zypper -n install -t pattern fsp-boost    # installs 6 Boost packages
\end{lstlisting}
% end_fsp_run
