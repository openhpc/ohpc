...
% begin_fsp_run
% fsp_validation_comment Use internal Centos mirror (CI only)
% [master]$ perl -pi -e 's!^YUM_MIRROR=\S+!YUM_MIRROR=''http://10.23.187.83/cobbler/ks_mirror/CentOS6.5-x86_64/''!' /usr/libexec/warewulf/wwmkchroot/centos-6.tmpl
% end_fsp_run
...
% begin_fsp_run
% fsp_validation_comment Update  BOS yum repo for compute image
\begin{lstlisting}[language=bash,literate={-}{-}1,keywords={},upquote=true]
...
[master]$ yum -y install lustre-client lustre-modules
...
[master]$ chroot $CHROOT yum -y install lustre-client lustre-modules
