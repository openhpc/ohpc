...
locally. Portions of the \texttt{/etc/hosts} file on the {\em master} host will
be automatically propagated to the provisioned hosts and a hostname entry that
maps to this {\em eth1} interface can be added as follows:
...
resource manager server, and enable NFS mounting of a \$HOME file system and the
public FSP install path (\texttt{/opt/fsp/pub}) that will be hosted by the {\em master} host in this example configuration. Note that the example shown here
...
[master]$ echo "/home <internal_netmask>(ro,no_subtree_check,fsid=0)" >> /etc/exports
[master]$ echo "/opt/fsp/pub <internal_netmask>(ro,no_subtree_check,fsid=0)" >> /etc/exports
[master]$ exportfs -a
[master]$ service nfsserver reload
\end{lstlisting}
% end_fsp_run
...
\begin{lstlisting}[language=bash,keywords={},upquote=true]
[master]$ zypper -n install intel-licenses
\end{lstlisting}
...
\S\ref{sec:add_rm} that can now be used to run an example test job.  The source
code for a simple ``hello-world'' MPI application for this example is
highlighted below. To use this account to compile the application and execute
it interactively through the resource manager under the default FSP
environment, execute the following.

\begin{lstlisting}[language=bash,keywords={}]
[master]$ su - test

# show "hello-world" source
[test@master ~]$ cat hello.c
#include ``mpi.h''
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
[test@master ~]$ mpicc -o hello -O3 hello.c
