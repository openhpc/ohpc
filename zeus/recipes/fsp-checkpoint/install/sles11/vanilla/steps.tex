...
# Export /home and FSP public packages  to cluster compute nodes
...
able to run jobs. The Zeus system includes a "test" user account housed on the
...
%Recall that we added a "test" user on the {\em master} host
%in \S\ref{sec:add_rm} 
This account is pre-populated with a simple "hello-world" MPI application and the source code
for this example is highlighted below. To use this account to compile the application and
execute it interactively through the resource manager, execute the following.
