...
show_usage() {
	...
	echo ""
	echo "version can be:"
	echo "  major         (e.g., '3') - finds newest 3.x version"
	echo "  major.minor   (e.g., '3.1') - uses specific version"
	echo "  major.minor.micro (e.g., '3.1.0') - uses specific version"
...
if [[ ${USE_FACTORY} -eq 1 ]]; then
	...
	if [[ ${num_parts} -eq 1 ]]; then
		...
		# For major-only versions with .0 (like "3.0"), validate the base version exists
...
if [[ ${USE_FACTORY} -eq 1 ]]; then
	...
	for release in ${releases}; do
		...
		for os in ${oses}; do
			# Construct factory repository URL
